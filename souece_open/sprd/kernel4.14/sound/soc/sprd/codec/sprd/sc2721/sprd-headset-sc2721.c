...
static void headset_detect_reg_init(int jack_type)
{
...
static void headset_detect_init(int jack_type)
{
	int val,val1;
	...
	if(JACK_TYPE_NC == jack_type)
	{
		headset_reg_set_bits(ANA_PMU0, BIT(HMICBIAS_VREF_SEL));
		...
		headset_reg_read(ANA_HDT0, &val1);
	}
	headset_reg_read(ANA_PMU0, &val);
	pr_info("%s: ANA_PMU0: %#x  ANA_HDT0: %#x\n", __func__, val,val1);
...
static int headset_sts0_confirm(u32 jack_type)
{
	if (headset_reg_get_bits(ANA_STS7, BIT(HEAD_INSERT_ALL)) == 0) {
		if (jack_type == JACK_TYPE_NC)
			return 1;          // zhou change_headset_detect 2019-9-30
		else
			return 1;
	} else {
		return 1;
...
retry_again:
	...
	pr_info("adc_mic_average = %d, adc_mic_ideal = %d\n", adc_mic_average,adc_mic_ideal);
	/*normal close case ,HP_DET disconnected from jack left,so
	* there is no chance to judge ERR cases*/
	// zhou change_headset_detect 2019-9-30
	if(pdata->jack_type == JACK_TYPE_NC) {
		if(adc_mic_average < 3200)
	...
	if (button_state_current == 1) {/* pressed! */
		if (pdata->nbuttons > 0) {
			...
			if (-1 == adc_mic_average) {
				...
				if (adc_mic_average > 80 && adc_mic_average < 300) {
					pr_info("glitch when plug out, not real press!)\n");
					goto out;
				}
...
static void headset_detect_all_work_func(struct work_struct *work)
{
	...
	pr_info("%s plug_state_current=%d plug_stat_last=%d enter into\n", __func__,plug_state_current,hdst->plug_stat_last);
	...
	if (hdst->plug_stat_last == 0) {
		...
		headset_detect_init(pdata->jack_type);
		msleep(20);
	}
	...
	if (hdst->plug_stat_last == 0) {
		gpio_detect_value_current =
			...
			if (pdata->jack_type == JACK_TYPE_NC) {
				if (gpio_detect_value_current == 0)
					gpio_detect_value_current = 1;
				else 
					gpio_detect_value_current = 0;
			}
	...
	pr_info("headphone zhou plug_state_current=%d plug_stat_last=%d re_detect=%d gpio_detect_value=%d \n",plug_state_current,hdst->plug_stat_last,
	hdst->re_detect,gpio_detect_value);
	...
	if ((1 == plug_state_current && 0 == hdst->plug_stat_last) ||
	  (hdst->re_detect == true && 1 == gpio_detect_value)) {
		...
		case HEADSET_4POLE_NOT_NORMAL:
			...
			break;	    // zhou change_headset_detect 2019-9-30		
...
static irqreturn_t headset_detect_all_irq_handler(int irq, void *dev)
{
	...
	hdst->gpio_det_val_last =
		gpio_get_value(pdata->gpios[HDST_GPIO_DET_ALL]);
	if (pdata->jack_type == JACK_TYPE_NC) {
		if (hdst->gpio_det_val_last == 0)
			hdst->gpio_det_val_last = 1;
		else 
			hdst->gpio_det_val_last = 0;
	}
	headset_reg_read(ANA_STS7, &val);
	...
	//if (pdata->jack_type == JACK_TYPE_NC) {     // zhou change_headset_detect 2019-9-30 start
	...
	//}         // zhou change_headset_detect 2019-9-30 end
#endif
...
int sprd_headset_soc_probe(struct snd_soc_codec *codec)
{
	...
	headset_detect_reg_init(pdata->jack_type);
