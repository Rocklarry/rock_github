...
#ifdef CONFIG_ENABLE_DOUBLE_TOUCHSCREEN
#define GOODIX_DEV_NAME2 "Goodix-CTP2"

static int hall_notifier_callback(struct notifier_block *self,
                unsigned long event, void *data);

extern int hall_notifier_register_client(struct notifier_block *nb);

extern int hall_notifier_unregister_client(struct notifier_block *nb);
#endif
...
*/
static DEVICE_ATTR(gesture, (S_IRUGO | S_IWUSR | S_IWGRP),
               gesture_switch_show,
               gesture_switch_store);
...
#ifdef CONFIG_ENABLE_DOUBLE_TOUCHSCREEN
#define GTP_DEBUGFS_DIR_14         "ts_debug_14"
#define GTP_DEBUGFS_DIR_5D         "ts_debug_5d"
#else
...
int gtp_send_cfg(struct goodix_ts_data *ts)
{
    ...
    if (ts->pdata->driver_send_cfg) {
        if (ts->config_data[ts->cfg_index] == NULL) {
            dev_err(&ts->client->dev,
                    "cfg-data?-%d matches with NULL , gtp_send_cfg error!\n", ts->cfg_index);
            return -1;
        }
        ...
        int w)
{
    ...
    dev_dbg(&ts->client->dev,
            "touch down id:%d, point:(%d, %d)", id, x, y);
...
//printk("stephen go to goodix_ts_work_func\n");
#ifdef CONFIG_FIBOCOM_GT9XX_TOUCHPANEL_UPDATE
//printk("stephen CONFIG_FIBOCOM_GT9XX_TOUCHPANEL_UPDATE\n");
    if (ts->enter_update)
        return;
#endif

    if (ts->pdata->slide_wakeup) {
//printk("stephen go to goodix_ts_work_func1\n");
        if (DOZE_ENABLED == ts->gs_state) {
//printk("stephen go to goodix_ts_work_func2\n");
            ret = gtp_i2c_read(ts->client, doze_buf, 3);
            if (ret > 0) {
//printk("stephen go to goodix_ts_work_func3\n");
                code = ts->pdata->keymap[doze_buf[2]];
                dev_info(&ts->client->dev, "reg 0x814B = 0x%02X, map keycode : 0x%x", doze_buf[2], code);
                if (code != 0) {
//printk("stephen go to goodix_ts_work_func4\n");
                    input_report_key(
                        ts->input_dev, code, 1);
...
//printk("stephen go to goodix_ts_work_func5\n");
            /* clear 0x814B */
            doze_buf[2] = 0x00;
...
//printk("stephen go to goodix_ts_work_func6\n");
    ret = gtp_i2c_read(ts->client, point_data, 12);
    if (ret < 0) {
//printk("stephen go to goodix_ts_work_func7\n");
        dev_err(&ts->client->dev,
                "I2C transfer error. errno:%d\n ", ret);
        goto exit_work_func;
    }
//printk("stephen go to goodix_ts_work_func8\n");
...
//printk("stephen go to goodix_ts_work_func9\n");
    }

//printk("stephen go to goodix_ts_work_func10\n");
    ...
    input_sync(ts->input_dev);
//printk("stephen go to goodix_ts_work_func11\n");
exit_work_func:
...
static irqreturn_t goodix_ts_irq_handler(int irq, void *dev_id)
{
    ...
    gtp_irq_disable(ts);
//printk("stephen go to goodix_ts_irq_handler\n");
...
{
	//  gpio_direction_output(ts->pdata->irq_gpio, 0);//modify stephen addr 
    msleep(ms);
    // gpio_direction_input(ts->pdata->irq_gpio); //modify stephen addr
}
...
printk("stephen gtp_reset_guitar 1\n");
printk("stephen ts->pdata->irq_gpio = %d\n,ts->pdata->reset_gpio = %d\n",ts->pdata->irq_gpio,ts->pdata->reset_gpio);
    gpio_direction_output(ts->pdata->reset_gpio, 0);
    msleep(ms);

    if (ts->client->addr == GTP_I2C_ADDRESS_HIGH)
	{
        //gpio_direction_output(ts->pdata->irq_gpio, 1); //modify stephen addr
	printk("stephen gpio_direction_output GTP_I2C_ADDRESS_HIGH\n");
	}
    else {
       // gpio_direction_output(ts->pdata->irq_gpio, 0);
	printk("stepen gpio_direction_output GTP_I2C_ADDRESS_LOW \n");//modify stephen  addr 
	}

   // usleep(RESET_DELAY_T3_US); //modify stephen 
...
printk("stephen go to gtp_enter_sleep \n");
printk("stephen ts->pdata->irq_gpio = %d\n",ts->pdata->irq_gpio);
//stephen modify addr 
/*
...
printk("stephen GTP sleep: Cannot reconfig gpio\n");
	}
*/
    if (ts->pdata->enable_power_off) {
printk("stephen gpio_direction_output ts->pdata->reset_gpio\n");
        ...
        if (ret)
		{
            dev_err(&ts->client->dev,
                "GTP sleep: Cannot reconfig gpio %d.\n",
                ts->pdata->reset_gpio);
printk("stephen GTP sleep: Cannot reconfig gpio %d.\n",ts->pdata->reset_gpio);
		}
        ret = goodix_power_off(ts);
        if (ret) {
printk("stephen GTP power off failed\n");
            dev_err(&ts->client->dev, "GTP power off failed.\n");
            return ret;
        }
        return 0;
    } else {
printk("stephen usleep_range(5000, 5500); \n");
       // usleep(5000);//modify stephen 
		usleep_range(5000, 5500);
        while (retry++ < GTP_I2C_RETRY_5) {
            ret = gtp_i2c_write(ts->client, i2c_control_buf, 3);
            if (ret == 1) {
				printk("stepen GTP enter sleep!\n");
...
static s8 gtp_wakeup_sleep(struct goodix_ts_data *ts)
{
	...
	printk("stephen got to gtp_wakeup_sleep\n");
    if (ts->pdata->enable_power_off) {
printk("stephen ts->pdata->irq_gpio, 0\n");
//modify stephen addr 
       // ret = gpio_direction_output(ts->pdata->irq_gpio, 0);
        if (ret){
            dev_err(&ts->client->dev,
                ...
                ts->pdata->irq_gpio);
printk("stephen GTP wakeup: Cannot reconfig gpio %d.\n",ts->pdata->irq_gpio);
		}
        ret = gpio_direction_output(ts->pdata->reset_gpio, 0);
        if (ret){
            dev_err(&ts->client->dev,
...
printk("stephen GTP wakeup: Cannot reconfig gpio %d.\n",ts->pdata->reset_gpio);
        ...
        if (ret) {
printk("stephen GTP power on failed.");
            dev_err(&ts->client->dev, "GTP power on failed.\n");
            return 0;
        }

        gtp_reset_guitar(ts, 20);
printk("stephen gtp_send_cfg\n");
        ret = gtp_send_cfg(ts);
        if (ret <= 0) {
            dev_err(&ts->client->dev,
                "GTP wakeup sleep send cfg failed.\n");
printk("stephen GTP wakeup sleep send cfg failed.\n");
            return ret;
        }
...
err_retry:
printk("stephen ts->pdata->slide_wakeup \n");
        if (ts->pdata->slide_wakeup
                && ts->gs_state != GS_DISABLED) {
printk("stephen else 1");
//modify stephen addr 
      //      ret = gpio_direction_output(ts->pdata->irq_gpio, 0);
            if (ret)
                dev_err(&ts->client->dev,
...
//modify stephen addr 
        ...
        if (ret == 2) {
            ...
            if (!ts->pdata->slide_wakeup || (ts->pdata->slide_wakeup &&
                    ts->gs_state == GS_DISABLED)) {
                if (ts->chip_gt9xxs == 0) {
                    gtp_int_sync(ts, 25);
                    msleep(20);
	...
	printk("stephen ======my init on =====\n");
	//===add stephen +++++
	//gtp_request_io_port(ts);
	//goodix_power_init(ts);
    //goodix_power_on(ts);
    //gtp_reset_guitar(ts, 20);
   //add stephen ------
    return ret;
...
static int gtp_init_panel(struct goodix_ts_data *ts)
{
    ...
    unsigned char *config_data[GOODIX_CFG_GROUP_NUM];
    int ret = -EIO;
    int i, n;
    u8 check_sum = 0;
    u8 opr_buf[16];
    ...
    if (ts->pdata->driver_send_cfg) {
        for (i = 0; i < GOODIX_MAX_CFG_GROUP; i++) {
            for (n = 0; n < GOODIX_CFG_GROUP_NUM; n++) {
                dev_dbg(&client->dev, "Config Groups(%d) Number(%d) Lengths: %d",//stephen modify 
                    i, n, (int)ts->pdata->config_data_len[i][n]);
            }
        }
		...
		dev_info(&client->dev, "sensor_id = %d\n", sensor_id);

        for (n = 0; n < GOODIX_CFG_GROUP_NUM; n++) {
            if (ts->pdata->config_data_len[sensor_id][n]) {
                    break;
            }
        }
        if (n >= GOODIX_CFG_GROUP_NUM) {
            dev_err(&client->dev,
                "Failed to get cfg-data, No Config Sent!");
            return -EINVAL;
        }
        ts->cfg_index = n;

        for (n = 0; n < GOODIX_CFG_GROUP_NUM; n++) {
            if (ts->pdata->config_data_len[sensor_id][n] < GTP_CONFIG_MIN_LENGTH ||
                    ts->pdata->config_data_len[sensor_id][n] > GTP_CONFIG_MAX_LENGTH ||
                    !ts->pdata->config_data[sensor_id][n]) {
                dev_err(&client->dev,
                        "Sensor_ID(%d) number(%d) matches with NULL or invalid config group!\n",
                        sensor_id, n);
            } else {
                dev_info(&client->dev, "Valid cfg-data%d-%d , config length: %d", (int)sensor_id, n, (int)ts->pdata->config_data_len[sensor_id][n]); //stephen modify
            }
        }
        ...
        if (ret == SUCCESS) {
            if (opr_buf[0] < 90) {
                ...
                /*for (n = 0; n < GOODIX_CFG_GROUP_NUM; n++) {
                    if (ts->pdata->config_data[sensor_id][n] != NULL) {
                        ts->pdata->config_data[sensor_id][n][GTP_ADDR_LENGTH] = 0x00;
                    }
                }*/
                ts->fixed_cfg = 0;
            } else {
        ...
        for (n = 0; n < GOODIX_CFG_GROUP_NUM; n++) {
            config_data[n] = ts->pdata->config_data[sensor_id][n];
            ts->config_data[n] = config_data[n];
            ts->gtp_cfg_len[n] = ts->pdata->config_data_len[sensor_id][n];

            if (config_data[n] != NULL) {
                #if GTP_CUSTOM_CFG
                config_data[n][RESOLUTION_LOC] =
                (unsigned char)(GTP_MAX_WIDTH && 0xFF);
                 config_data[n][RESOLUTION_LOC + 1] =
                 (unsigned char)(GTP_MAX_WIDTH >> 8);
                config_data[n][RESOLUTION_LOC + 2] =
                (unsigned char)(GTP_MAX_HEIGHT && 0xFF);
                config_data[n][RESOLUTION_LOC + 3] =
                (unsigned char)(GTP_MAX_HEIGHT >> 8);

                if (GTP_INT_TRIGGER == 0)
                    config_data[n][TRIGGER_LOC] &= 0xfe;
                else if (GTP_INT_TRIGGER == 1)
                    config_data[n][TRIGGER_LOC] |= 0x01;
                #endif  /* !GTP_CUSTOM_CFG */

                check_sum = 0;
                for (i = GTP_ADDR_LENGTH; i < ts->gtp_cfg_len[n]; i++)
                    check_sum += config_data[n][i];

                config_data[n][ts->gtp_cfg_len[n]] = (~check_sum) + 1;
            }
        }
    } else { /* DRIVER NOT SEND CONFIG */
        ts->gtp_cfg_len[ts->cfg_index] = GTP_CONFIG_MAX_LENGTH;

        config_data[ts->cfg_index] = devm_kzalloc(&client->dev,
                GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH,
                GFP_KERNEL);

        ret = gtp_i2c_read(ts->client, config_data[ts->cfg_index],
            ts->gtp_cfg_len[ts->cfg_index] + GTP_ADDR_LENGTH);
        if (ret < 0) {
            dev_err(&client->dev,
    ...
    dev_info(&client->dev, "cfg-data%d-%d used, config length: %d", (int)sensor_id, (int)ts->cfg_index,
            (int)ts->pdata->config_data_len[sensor_id][ts->cfg_index]);

    if ((ts->abs_x_max == 0) && (ts->abs_y_max == 0)) {
        ts->abs_x_max = (config_data[ts->cfg_index][RESOLUTION_LOC + 1] << 8)
                + config_data[ts->cfg_index][RESOLUTION_LOC];
        ts->abs_y_max = (config_data[ts->cfg_index][RESOLUTION_LOC + 3] << 8)
                + config_data[ts->cfg_index][RESOLUTION_LOC + 2];
        ts->int_trigger_type = (config_data[ts->cfg_index][TRIGGER_LOC]) & 0x03;
    }
    ret = gtp_send_cfg(ts);
    if (ret <= 0)
...
static int gtp_check_product_id(struct i2c_client *client)
{
    ...
    sscanf(product_id, "%d", &id);
    pid = ts->pdata->product_id;
    num_id = ts->pdata->num_product_id;
...
static int gtp_request_io_port(struct goodix_ts_data *ts)
{
    ...
    int ret;
	printk("stephen go to gtp_request_io_port\n");
	printk("stephen gtp_request_io_port gpio_is_valid = %d\n",pdata->irq_gpio);
    if (gpio_is_valid(pdata->irq_gpio)) {
	printk("stephen gpio_is_valid 1\n");
        ret = gpio_request(pdata->irq_gpio, "goodix_ts_irq_gpio");
        if (ret) {
	printk("stephen gpio_request 1xx\n");
            dev_err(&client->dev, "irq gpio request failed\n");
            goto pwr_off;
        }
	printk("stephen gtp_request_io_port 2\n");
        ret = gpio_direction_output(pdata->irq_gpio, 0);
        if (ret) {
	printk("stephen gtp_request_io_port 3\n");
            dev_err(&client->dev,
                    "set_direction and pull down for irq gpio failed\n");
            goto free_irq_gpio;
        }
    } else {
	printk("stephen gtp_request_io_port 4\n");
        dev_err(&client->dev, "irq gpio is invalid!\n");
        ret = -EINVAL;
        goto free_irq_gpio;
    }
printk("stephen gtp_request_io_port 5\n");
    if (gpio_is_valid(pdata->reset_gpio)) {
printk("stephen gtp_request_io_port 6\n");
        ret = gpio_request(pdata->reset_gpio, "goodix_ts_reset_gpio");
        if (ret) {
printk("stephen gtp_request_io_port 7\n");
            dev_err(&client->dev, "reset gpio request failed\n");
            goto free_irq_gpio;
        }
printk("stephen gtp_request_io_port 8\n");
        ret = gpio_direction_output(pdata->reset_gpio, 0);
        if (ret) {
printk("stephen gtp_request_io_port 9\n");
            dev_err(&client->dev,
                    "set_direction and pull down for reset gpio failed\n");
            goto free_reset_gpio;
        }
    } else {
printk("stephen gtp_request_io_port 10\n");
        dev_err(&client->dev, "reset gpio is invalid!\n");
        ret = -EINVAL;
        goto free_reset_gpio;
    }
printk("stephen gtp_request_io_port 11\n");
    if (gpio_is_valid(pdata->vdd_gpio)) {
printk("stephen gtp_request_io_port 12\n");
        ret = gpio_request(pdata->vdd_gpio, "goodix_ts_vdd_gpio");
        if (ret) {
printk("stephen gtp_request_io_port 13\n");
            dev_err(&client->dev, "vdd gpio request failed\n");
            goto free_reset_gpio;
        }
printk("stephen gtp_request_io_port 14\n");
        ret = gpio_direction_output(pdata->vdd_gpio, 0);
        if (ret) {
printk("stephen gtp_request_io_port 15\n");
            dev_err(&client->dev,
                    "set_direction and pull down for vdd gpio failed\n");
            goto free_vdd_gpio;
        }
    } else {
printk("stephen gtp_request_io_port 16\n");
        dev_info(&client->dev, "vdd gpio is invalid!\n");
    }
printk("stephen gtp_request_io_port 17\n");
    return ret;
...
printk("stephen gtp_request_irq ts->client->irq = %d\n",ts->client->irq);
    ret = request_threaded_irq(ts->client->irq, NULL,
            goodix_ts_irq_handler,
            ts->pdata->irq_gpio_flags,  /* irq_table[ts->int_trigger_type] | IRQF_ONESHOT, */
            ts->client->name, ts);
    if (ret) {
        ts->use_irq = false;
        return ret;
...
static int gtp_request_input_dev(struct goodix_ts_data *ts)
{
    ...
    if (ts->pdata->slide_wakeup) {
        for (index = 0; index < ts->pdata->keymap_len; index++) {
            if (ts->pdata->keymap[index] != 0) {
                input_set_capability(ts->input_dev, EV_KEY,
                        ts->pdata->keymap[index]);
            }
        }
    }
	...
	if(!ts->abs_x_max/* || (ts->abs_x_max != ts->pdata->x_max)*/)
		ts->abs_x_max = ts->pdata->x_max;

	if(!ts->abs_y_max/* || (ts->abs_y_max != ts->pdata->y_max)*/)
    ...
    if (ts->client->addr == GTP_I2C_ADDRESS_HIGH) {
        ...
        ts->input_dev->name = GOODIX_DEV_NAME2;
    }
#else
    ts->input_dev->name = GOODIX_DEV_NAME;
#endif
...
{
    int ret;
printk("stephen goodix_power_on \n");
    if (ts->power_on) {
printk("stephen goodix_power_on 1\n");
        dev_info(&ts->client->dev,
                "Device already power on\n");
        return 0;
    }
printk("stephen goodix_power_on 2\n");
    if (!IS_ERR(ts->avdd)) {
printk("stephen goodix_power_on 3\n");
        ret = reg_set_optimum_mode_check(ts->avdd,
            GOODIX_VDD_LOAD_MAX_UA);
        if (ret < 0) {
printk("stephen goodix_power_on 4\n");
            dev_err(&ts->client->dev,
                "Regulator avdd set_opt failed rc=%d\n", ret);
            goto err_set_opt_avdd;
        }
printk("stephen goodix_power_on 5\n");
        ret = regulator_enable(ts->avdd);
        if (ret) {
printk("stephen goodix_power_on 6\n");
            dev_err(&ts->client->dev,
                "Regulator avdd enable failed ret=%d\n", ret);
            goto err_enable_avdd;
        }
    }
printk("stephen goodix_power_on 7\n");
    if (!IS_ERR(ts->vdd)) {
printk("stephen goodix_power_on 8\n");
        ret = regulator_set_voltage(ts->vdd, GOODIX_VTG_MIN_UV,
                       GOODIX_VTG_MAX_UV);
        if (ret) {
printk("stephen goodix_power_on 9\n");
            dev_err(&ts->client->dev,
                "Regulator set_vtg failed vdd ret=%d\n", ret);
            goto err_set_vtg_vdd;
        }
printk("stephen goodix_power_on 10\n");
        ret = reg_set_optimum_mode_check(ts->vdd,
            GOODIX_VDD_LOAD_MAX_UA);
        if (ret < 0) {
printk("stephen goodix_power_on 11\n");
            dev_err(&ts->client->dev,
                "Regulator vdd set_opt failed rc=%d\n", ret);
            goto err_set_opt_vdd;
        }
printk("stephen goodix_power_on 12\n");
        ret = regulator_enable(ts->vdd);
        if (ret) {
printk("stephen goodix_power_on 13\n");
            dev_err(&ts->client->dev,
                "Regulator vdd enable failed ret=%d\n", ret);
            goto err_enable_vdd;
        }
    }
printk("stephen goodix_power_on 14\n");
    if (gpio_is_valid(ts->pdata->vdd_gpio)) {
printk("stephen goodix_power_on 15\n");
        ret = gpio_direction_output(ts->pdata->vdd_gpio, 1);
        if (ret) {
printk("stephen goodix_power_on 16\n");
            dev_err(&ts->client->dev,
                "GTP wakeup: Cannot reconfig gpio %d.\n",
                ts->pdata->vdd_gpio);
            goto err_enable_vdd;
...
printk("stephen goodix_power_on 23\n");
        ret = regulator_enable(ts->vcc_i2c);
        if (ret) {
printk("stephen goodix_power_on 24\n");
            dev_err(&ts->client->dev,
...
printk("stephen goodix_power_on 25\n");
    ts->power_on = true;
    return 0;
...
static int goodix_power_off(struct goodix_ts_data *ts)
{
	...
	printk("stephen goodix_power_off \n");
    if (!ts->power_on) {
printk("stephen goodix_power_off 1\n");
        dev_info(&ts->client->dev,
                "Device already power off\n");
        return 0;
    }
printk("stephen goodix_power_off 2\n");
    if (!IS_ERR(ts->vcc_i2c)) {
printk("stephen goodix_power_off 3\n");
    ...
    if (!IS_ERR(ts->vdd)) {
printk("stephen goodix_power_off 4\n");
    ...
    if (gpio_is_valid(ts->pdata->vdd_gpio)) {
printk("stephen goodix_power_off 5\n");
        ret = gpio_direction_output(ts->pdata->vdd_gpio, 0);
        if (ret) {
            dev_err(&ts->client->dev,
                "GTP wakeup: Cannot reconfig gpio %d.\n",
                ts->pdata->vdd_gpio);
...
printk("stephen goodix_power_off 6\n");
    if (!IS_ERR(ts->avdd)) {
printk("stephen goodix_power_off 7\n");
        ret = regulator_disable(ts->avdd);
        if (ret)
            dev_err(&ts->client->dev,
                "Regulator avdd disable failed ret=%d\n", ret);
    }
printk("stephen goodix_power_off 8\n");
    ts->power_on = false;
    return 0;
}
...
printk("stephen goodix_power_init \n");
    ts->avdd = regulator_get(&ts->client->dev, "avdd");
    if (IS_ERR(ts->avdd)) {
printk("stephen goodix_power_init 1\n");
        ret = PTR_ERR(ts->avdd);
        dev_info(&ts->client->dev,
            "Regulator get failed avdd ret=%d\n", ret);
    }
printk("stephen goodix_power_init 2\n");
    ts->vdd = regulator_get(&ts->client->dev, "vdd");
    if (IS_ERR(ts->vdd)) {
printk("stephen goodix_power_init 3\n");
        ret = PTR_ERR(ts->vdd);
        dev_info(&ts->client->dev,
            "Regulator get failed vdd ret=%d\n", ret);
    }
printk("stephen goodix_power_init 4\n");
    ts->vcc_i2c = regulator_get(&ts->client->dev, "vcc_i2c");
    if (IS_ERR(ts->vcc_i2c)) {
printk("stephen goodix_power_init 5\n");
                ...
                const char *buf, size_t size)
{
    ...
    if (!ts->fw_loading && val) {
        ...
        ts->fw_loading = true;
        if (config_enabled(CONFIG_FIBOCOM_GT9XX_TOUCHPANEL_UPDATE)) {
            ret = gup_update_proc(NULL, ts->client);
            if (ret == FAIL)
                dev_err(&ts->client->dev,
...
{
#ifdef CONFIG_ENABLE_DOUBLE_TOUCHSCREEN
    if (data->client->addr == GTP_I2C_ADDRESS_HIGH) {
        data->debug_base = debugfs_create_dir(GTP_DEBUGFS_DIR_14, NULL);
    } else {
        data->debug_base = debugfs_create_dir(GTP_DEBUGFS_DIR_5D, NULL);
    }
#else
            ...
            struct goodix_ts_platform_data *pdata)
{
    ...
    struct device_node *np = dev->of_node;
    struct property *prop;
    u32 temp_val, num_buttons, num_id;
    ...
    if (pdata->slide_wakeup) {
        ...
        if (!prop || !prop->value || key_map_len <= 0) {
            dev_err(dev, "key-code-map not specified\n");
            return  -EINVAL;
        }

        dev_info(dev, "use gesture keycode map, map len:%d.\n", key_map_len);

        pdata->keymap = devm_kzalloc(dev, key_map_len,
                                GFP_KERNEL);

        pdata->keymap_len = key_map_len;
    ...
    pdata->vdd_gpio = of_get_named_gpio_flags(np, "vdd-gpios",
                0, &pdata->vdd_gpio_flags);
    if (pdata->vdd_gpio < 0)
        dev_info(dev, "Can't find gpio name vdd-gpios.\n");

    prop = of_find_property(np, "goodix,product-id", NULL);
    if (prop) {
        num_id = prop->length / sizeof(temp_val);
        if (num_id > GOODIX_MAX_PID_GROUP) {
            dev_err(dev, "num_pid = %d  is larger than GOODIX_MAX_PID_GROUP\n", num_id);
            return -EINVAL;
        }

        pdata->product_id = devm_kzalloc(dev, num_id*sizeof(temp_val),
                GFP_KERNEL);
        pdata->num_product_id = num_id;

        rc = of_property_read_u32_array(np,
            "goodix,product-id", pdata->product_id,
            num_id);

        if (rc) {
            dev_err(dev, "Unable to read product_id\n");
            return rc;
        }
    }
    ...
    for (i = 0; i < GOODIX_MAX_CFG_GROUP; i++) {
        for (n = 0; n < GOODIX_CFG_GROUP_NUM; n++) {
            snprintf(prop_name, sizeof(prop_name), "goodix,cfg-data%d-%d", i, n);

            prop = of_find_property(np, prop_name,
                (int *)&pdata->config_data_len[i][n]);//modify stephen  &pdata->config_data_len[i][n] ---> (int *)&pdata->config_data_len[i][n]
            if (!prop || !prop->value) {
                pdata->config_data_len[i][n] = 0;
                pdata->config_data[i][n] = NULL;
                continue;
            }

           pdata->config_data[i][n] = devm_kzalloc(dev,
                        GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH,
                        GFP_KERNEL);

            if (!pdata->config_data[i][n]) {
                dev_err(dev,
                    "Not enough memory for panel config data %d\n",
                    i);
                return -ENOMEM;
            }
            pdata->config_data[i][n][0] = GTP_REG_CONFIG_DATA >> 8;
            pdata->config_data[i][n][1] = GTP_REG_CONFIG_DATA & 0xff;
            memcpy(&pdata->config_data[i][n][GTP_ADDR_LENGTH],
                    prop->value, pdata->config_data_len[i][n]);
            read_cfg_num++;
        }
    }
...
printk("stephen GTP I2C Address: 0x%02x\n", client->addr);
    if (client->dev.of_node) {
printk("stephen goodix_ts_probe =============\n");
        pdata = devm_kzalloc(&client->dev,
            sizeof(struct goodix_ts_platform_data), GFP_KERNEL);
        if (!pdata) {
printk("stephen goodix_ts_probe =============1\n");
            dev_err(&client->dev,
                "GTP Failed to allocate memory for pdata\n");
            return -ENOMEM;
        }
printk("stephen goodix_ts_probe =============2\n");
        ret = goodix_parse_dt(&client->dev, pdata);
        if (ret)
            return ret;
    } else {
printk("stephen goodix_ts_probe =============3\n");
        pdata = client->dev.platform_data;
    }
printk("stephen goodix_ts_probe =============4\n");
    if (!pdata) {
printk("stephen goodix_ts_probe =============5\n");
        dev_err(&client->dev, "GTP invalid pdata\n");
        return -EINVAL;
    }
printk("stephen goodix_ts_probe =============6\n");
    if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
printk("stephen goodix_ts_probe =============7\n");
        dev_err(&client->dev, "GTP I2C not supported\n");
        return -ENODEV;
    }
printk("stephen goodix_ts_probe =============8\n");
    ts = devm_kzalloc(&client->dev, sizeof(*ts), GFP_KERNEL);
    if (!ts) {
printk("stephen goodix_ts_probe =============9\n");
        dev_err(&client->dev, "GTP not enough memory for ts\n");
        return -ENOMEM;
    }
printk("stephen goodix_ts_probe =============10\n");
     ...
     */
printk("stephen goodix_ts_probe =============11\n");
...
#ifdef CONFIG_ENABLE_DOUBLE_TOUCHSCREEN
printk("stephen goodix_ts_probe =============12\n");
    mutex_init(&ts->hall_lock);
    ts->hall_notif.notifier_call = hall_notifier_callback;
    ret = hall_notifier_register_client(&ts->hall_notif);
    if (ret) {
printk("stephen goodix_ts_probe =============13\n");
        dev_err(&ts->client->dev,
            "Unable to register hall_notifier: %d\n",
            ret);
    }
#endif
printk("stephen goodix_ts_probe =============14\n");
#ifdef CONFIG_TS_GLOVE_SWITCH
    mutex_init(&ts->glove_lock);
printk("stephen goodix_ts_probe =============15\n");
    wake_lock_init(&ts->wake_lock, WAKE_LOCK_SUSPEND, "ts_wakelock");
#endif
    ts->cfg_index = 0;
printk("stephen goodix_ts_probe =============16\n");
    ret = gtp_request_io_port(ts);
    if (ret) {
printk("stephen goodix_ts_probe =============17\n");
        dev_err(&client->dev, "GTP request IO port failed.\n");
        goto exit_free_client_data;
    }
printk("stephen goodix_ts_probe =============18\n");
    ret = goodix_power_init(ts);
    if (ret) {
printk("stephen goodix_ts_probe =============19\n");
        dev_err(&client->dev, "GTP power init failed\n");
        goto exit_free_io_port;
    }
printk("stephen goodix_ts_probe =============20\n");
    ret = goodix_power_on(ts);
    if (ret) {
printk("stephen goodix_ts_probe =============21\n");
        dev_err(&client->dev, "GTP power on failed\n");
        goto exit_deinit_power;
    }
printk("stephen goodix_ts_probe =============22\n");
    gtp_reset_guitar(ts, 20);
...
printk("stephen goodix_ts_probe =============23\n");
    if (pdata->force_update)
        ts->force_update = true;
printk("stephen goodix_ts_probe =============24\n");
    if (pdata->fw_name)
        strlcpy(ts->fw_name, pdata->fw_name,
                        strlen(pdata->fw_name) + 1);
printk("stephen goodix_ts_probe =============25\n");
    /*if (config_enabled(CONFIG_FIBOCOM_GT9XX_TOUCHPANEL_UPDATE)) {
    ...
    if (ret < 0) {
printk("stephen goodix_ts_probe =============26\n");
...
printk("stephen goodix_ts_probe =============27\n");
    ret = gtp_request_input_dev(ts);
    if (ret) {
printk("stephen goodix_ts_probe =============28\n");
        dev_err(&client->dev, "GTP request input dev failed.\n");
        goto exit_power_off;
    }
    input_set_drvdata(ts->input_dev, ts);
printk("stephen goodix_ts_probe =============29\n");
    mutex_init(&ts->lock);

#if defined(CONFIG_FB)
printk("stephen goodix_ts_probe =============30\n");
//modify stephen 
...
#elif defined(CONFIG_HAS_EARLYSUSPEND)
//modify stephen
printk("stephen goodix_ts_probe =============31\n");
    ts->early_suspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN + 1;
    ts->early_suspend.suspend = goodix_ts_early_suspend;
    ts->early_suspend.resume = goodix_ts_late_resume;
    register_early_suspend(&ts->early_suspend);
#endif
printk("stephen goodix_ts_probe =============32\n");
...
printk("stephen goodix_ts_probe =============33\n");
    if (ts->use_irq)
        gtp_irq_enable(ts);

#ifdef CONFIG_FIBOCOM_GT9XX_TOUCHPANEL_DEBUG
printk("stephen goodix_ts_probe =============34\n");
    init_wr_node(client);
#endif

#if GTP_ESD_PROTECT
printk("stephen goodix_ts_probe =============35\n");
    gtp_esd_switch(client, SWITCH_ON);
#endif
printk("stephen goodix_ts_probe =============36\n");
    ret = sysfs_create_group(&client->dev.kobj, &gtp_attr_grp);
    if (ret < 0) {
printk("stephen goodix_ts_probe =============37\n");
        dev_err(&client->dev, "sys file creation failed.\n");
        goto exit_free_irq;
    }
printk("stephen goodix_ts_probe =============38\n");
    ret = gtp_debugfs_init(ts);
    if (ret != 0) {
printk("stephen goodix_ts_probe =============39\n");
        dev_err(&client->dev, "Failed to create debugfs entries, %d\n",
                        ret);
        goto exit_remove_sysfs;
    }
printk("stephen goodix_ts_probe =============40\n");
#ifdef CONFIG_TS_GLOVE_SWITCH
    ret = device_create_file(&ts->input_dev->dev, &dev_attr_glove);
    if (ret) {
printk("stephen goodix_ts_probe =============41\n");
        dev_err(&client->dev, "device create glove file failed.\n");
        goto exit_create_glove;
    }
#endif
printk("stephen goodix_ts_probe =============42\n");
    if (pdata->slide_wakeup) {
printk("stephen goodix_ts_probe =============43\n");
        ret = device_create_file(&ts->input_dev->dev, &dev_attr_gesture);
        if (ret) {
printk("stephen goodix_ts_probe =============44\n");
            dev_err(&client->dev, "device create gesture file failed.\n");
            goto exit_create_gesture;
        }
        device_init_wakeup(&client->dev, 1);
    }
printk("stephen goodix_ts_probe =============45\n");
    ts->init_done = true;
    return 0;

exit_create_gesture:
#ifdef CONFIG_TS_GLOVE_SWITCH
    device_remove_file(&ts->input_dev->dev, &dev_attr_glove);
exit_create_glove:
#endif
...
exit_free_client_data:
#ifdef CONFIG_TS_GLOVE_SWITCH
    mutex_destroy(&ts->glove_lock);
    wake_lock_destroy(&ts->wake_lock);
#endif

#ifdef CONFIG_ENABLE_DOUBLE_TOUCHSCREEN
    if (hall_notifier_unregister_client(&ts->hall_notif)) {
        dev_err(&ts->client->dev,
            "Unable to register hall_notifier.\n");
    }
    mutex_destroy(&ts->hall_lock);
#endif
    ...
    for (i = 0 ; i < GOODIX_MAX_CFG_GROUP ; i++) {
        for (n = 0; n < GOODIX_CFG_GROUP_NUM; n++) {
            if (pdata->config_data[i][n] != NULL) {
                devm_kfree(&client->dev, pdata->config_data[i][n]);
            }
        }
    }
    if (pdata->slide_wakeup) {
        if (pdata->keymap && pdata->keymap_len)
            devm_kfree(&client->dev, pdata->keymap);
    }
    if (pdata->product_id) {
        devm_kfree(&client->dev, pdata->product_id);
    }
    if (client->dev.of_node) {
...
static int goodix_ts_remove(struct i2c_client *client)
{
    ...
    if (pdata->slide_wakeup) {
        ...
        device_remove_file(&ts->input_dev->dev, &dev_attr_gesture);
    }
...
#ifdef CONFIG_FIBOCOM_GT9XX_TOUCHPANEL_DEBUG
    uninit_wr_node(client);
#endif

#if GTP_ESD_PROTECT
    cancel_work_sync(ts->gtp_esd_check_workqueue);
    flush_workqueue(ts->gtp_esd_check_workqueue);
    destroy_workqueue(ts->gtp_esd_check_workqueue);
#endif
...
static int goodix_ts_suspend(struct device *dev)
{
    ...
    if (ts->pdata->slide_wakeup && ts->gs_state != GS_DISABLED) {
                 ...
                 unsigned long event, void *data)
{
    ...
    if (data) {
        mutex_lock(&ts->hall_lock);
        if (strcmp(data, "far") == 0) {
            ts->hall_state = 1;
        } else if (strcmp(data, "close") == 0) {
            ts->hall_state = 0;
        }
        mutex_unlock(&ts->hall_lock);
...
#ifdef CONFIG_TS_GLOVE_SWITCH
static int ts_glove_switch(struct goodix_ts_data *ts, int glove_state)
{
    int ret;

    wake_lock(&ts->wake_lock);
    mutex_lock(&ts->glove_lock);

    if (ts->cfg_index != glove_state) {
        ts->cfg_index = glove_state;
        if (!ts->gtp_is_suspend) {
            ret = gtp_send_cfg(ts);
            if (ret <= 0) {
                dev_err(&ts->client->dev, "%s: Send config error.\n", __func__);
            }
            msleep(300);
        }
    }

    mutex_unlock(&ts->glove_lock);
         ...
         const char *buf, size_t count)
{
    ...
    if (glove_state >= GOODIX_CFG_GROUP_NUM || glove_state < 0) {
        dev_err(&ts->client->dev, "switch to cfg_index : %d error\n", glove_state);
        return -EINVAL;
    }
    ts_glove_switch(ts, glove_state);
         ...
         const char *buf, size_t count)
{
    ...
    if (ts->gtp_is_suspend == false) {
        sscanf(buf, "%d", &gesture_state);
        if (gesture_state < 0) {
            dev_err(&ts->client->dev, "gesture_state switch to : %d error\n", gesture_state);
            return -EINVAL;
        }
...
void gtp_esd_switch(struct i2c_client *client, int on)
{
    ...
    if (SWITCH_ON == on) {
        ...
        if (!ts->esd_running) {
            ...
            queue_delayed_work(ts->gtp_esd_check_workqueue,
                &ts->gtp_esd_check_work, GTP_ESD_CHECK_CIRCLE);
        }
    } else {
        /* switch off esd */
        if (ts->esd_running) {
            ...
            cancel_delayed_work_sync(&ts->gtp_esd_check_work);
        }
    }
}
...
static void gtp_esd_check_func(struct work_struct *work)
{
    ...
    u8 test[4] = {0x80, 0x40};
    struct goodix_ts_data *ts =
            container_of(work, struct goodix_ts_data, gtp_esd_check_work);

    ts = i2c_get_clientdata(ts->client);
...
static const struct dev_pm_ops goodix_ts_dev_pm_ops = {
    .suspend = goodix_ts_suspend,//modify stephen
...
//    { .compatible = "goodix,gt9xx_14", },
//    { .compatible = "goodix,gt9xx_5d", },
//    { },
//};
//#else
static struct of_device_id goodix_match_table[] = {
...
static struct i2c_driver goodix_ts_driver = {
    ...
    .suspend    = goodix_ts_early_suspend,//modify stephen
    .resume     = goodix_ts_late_resume, //modify stephen
#endif
...
#if CONFIG_PM
        .pm = &goodix_ts_dev_pm_ops,//modify stephen
#endif
    },
};
...
#if GTP_ESD_PROTECT
    INIT_DELAYED_WORK(&ts->gtp_esd_check_work, gtp_esd_check_func);
    ts->gtp_esd_check_workqueue = create_workqueue("gtp_esd_check");
#endif
