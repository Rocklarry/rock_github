...
int gtp_i2c_read(struct i2c_client *client, u8 *buf, int len)
{
    ...
    for (retries = 0; retries < GTP_I2C_RETRY_3; retries++) {
        ret = i2c_transfer(client->adapter, msgs, 2);
        if (ret == 2)
            break;
        dev_err(&client->dev, "I2C retry: %d\n", retries + 1);
    }
    if (retries == GTP_I2C_RETRY_3) {
...
int gtp_send_cfg(struct goodix_ts_data *ts)
{
    ...
    if (ts->pdata->driver_send_cfg) {
        ...
        if (ts->fixed_cfg) {
            ...
            for (retry = 0; retry < GTP_I2C_RETRY_3; retry++) { //fanhang reduce retry times(5) to 3 @2018.6.11
...
#ifdef CONFIG_SS80X_TOUCHSCREEN_GT9XX_UPDATE
    if (ts->enter_update)
        return;
#endif
                ...
                const char *buf, size_t size)
{
    ...
    if (!ts->fw_loading && val) {
        ...
        ts->fw_loading = true;
        if (__is_defined(CONFIG_GOSO_GT9XX_TOUCHPANEL_UPDATE)) {
...
/****************************************************************/
//creat suspend sysfile for GTP
/****************************************************************/
static ssize_t gtp_suspend_show(struct device *dev,
	           struct device_attribute *attr, char *buf)
{
	struct goodix_ts_data *ts = dev_get_drvdata(dev);
	return sprintf(buf, "%s\n",
		ts->gtp_is_suspend ? "true" : "false");
}
static ssize_t gtp_suspend_store(struct device *dev,
	           struct device_attribute *attr, const char *buf, size_t count)
{
	struct goodix_ts_data *ts = dev_get_drvdata(dev);
	if ((buf[0] == '1') && !ts->gtp_is_suspend)
		goodix_ts_suspend(dev);
	else if ((buf[0] == '0') && ts->gtp_is_suspend)
		goodix_ts_resume(dev);
	return count;
}
#ifdef CONFIG_TS_GLOVE_SWITCH
...
static DEVICE_ATTR(ts_suspend, (S_IRUGO | S_IWUSR | S_IWGRP),  //Create a system file and write it out by LCD
            gtp_suspend_show,
            gtp_suspend_store);
            ...
            struct goodix_ts_platform_data *pdata)
{
    ...
    rc = of_property_read_string(np, "goodix,device_name",  //ethan gao add for device read
                        &pdata->device_name);
    if (rc && (rc != -EINVAL)) {
        dev_err(dev, "Failed to parse device_name.\n");
        return -EINVAL;
    }
...
static int goodix_ts_suspend(struct device *dev)
{
    ...
    mutex_lock(&ts->lock);
	dev_err(&ts->client->dev, "GTP enter suspend!!! \n");
    if (ts->fw_loading) {
    ...
    if (ts->pdata->slide_wakeup && ts->gs_state != GS_DISABLED) {
        ...
        if (ts->use_irq)
		//modified by Ethan gao ....start
		{
			free_irq(ts->client->irq,ts);
	   }
	  //modified by Ethan gao  ....end
...
static int goodix_ts_resume(struct device *dev)
{
    ...
    int ret = 0;
    if (!ts->gtp_is_suspend) {
        dev_err(&ts->client->dev, "Already in awake state.\n");
        return 0;
    }
	dev_err(&ts->client->dev, "GTP enter resume!!! \n");
    mutex_lock(&ts->lock);
	...
	gtp_irq_enable(ts);
	if (ret)
    dev_err(&ts->client->dev, "GTP request irq failed %d.\n", ret);
    else
    dev_err(&ts->client->dev, "GTP works in interrupt mode.\n");
	 }
   //modified by Ethan gao.... end
#if GTP_ESD_PROTECT
...
#ifdef CONFIG_SS80X_TOUCHSCREEN_GT9XX_DEBUG
    init_wr_node(client);
#endif
...
#if defined(CONFIG_FB)
    /* if (adf_unregister_client(&ts->adf_event_block))
        dev_err(&client->dev,
            "Error occurred while unregistering adf_event_block.\n"); */
#elif defined(CONFIG_HAS_EARLYSUSPEND)
