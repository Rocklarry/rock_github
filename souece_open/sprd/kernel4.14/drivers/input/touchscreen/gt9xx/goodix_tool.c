...
static s32 (*tool_i2c_read)(struct i2c_client *, u8 *, u16);
static s32 (*tool_i2c_write)(struct i2c_client *, u8 *, u16);

s32 data_length;
s8 ic_type[16] = {0};

static void tool_set_proc_name(struct goodix_ts_data *ts)
{
    ...
    //snprintf(date, 20, "%s", __DATE__);//modify stephen
    ...
    if (ts->client->addr == GTP_I2C_ADDRESS_HIGH) {
        snprintf(ts->procname, 20, "gmnode%04d%02d%02d_14", n_year, n_month, n_day);
    } else {
        snprintf(ts->procname, 20, "gmnode%04d%02d%02d_5d", n_year, n_month, n_day);
    }
#else
    snprintf(ts->procname, 20, "gmnode%04d%02d%02d", n_year, n_month, n_day);
#endif
    /* pr_debug("procname = %s", procname); */
}

static s32 tool_i2c_read_no_extra(struct i2c_client *client, u8 *buf, u16 len)
{
    s32 ret = -1;
    u8 i = 0;
...
static s32 tool_i2c_write_no_extra(struct i2c_client *client, u8 *buf, u16 len)
{
    s32 ret = -1;
    u8 i = 0;
    struct i2c_msg msg = {
...
static s32 tool_i2c_read_with_extra(struct i2c_client *client, u8 *buf, u16 len)
{
    s32 ret = -1;
    u8 pre[2] = {0x0f, 0xff};
    u8 end[2] = {0x80, 0x00};

    tool_i2c_write_no_extra(client, pre, 2);
    ret = tool_i2c_read_no_extra(client, buf, len);
    tool_i2c_write_no_extra(client, end, 2);

    return ret;
}

static s32 tool_i2c_write_with_extra(struct i2c_client *client, u8 *buf, u16 len)
{
    s32 ret = -1;
    u8 pre[2] = {0x0f, 0xff};
    u8 end[2] = {0x80, 0x00};
    ...
    tool_i2c_write_no_extra(client, pre, 2);
    ret = tool_i2c_write_no_extra(client, buf, len);
    tool_i2c_write_no_extra(client, end, 2);
...
void uninit_wr_node(struct i2c_client *client)
{
    struct goodix_ts_data *ts = i2c_get_clientdata(client);
    cmd_head.data = NULL;
    unregister_i2c_func();
    remove_proc_entry(ts->procname, NULL);
}
...
Output:
    Return write length.
********************************************************/
static u8 comfirm(struct i2c_client *client)
{
    s32 i = 0;
    u8 buf[32];
    ...
    for (i = 0; i < cmd_head.times; i++) {
        if (tool_i2c_read(client, buf, 1) <= 0) {
            dev_err(&client->dev, "Read flag data failed!");
            return FAIL;
        }
    ...
    if (i >= cmd_head.times) {
        dev_err(&client->dev, "Didn't get the flag to continue!");
        return FAIL;
    }
...
********************************************************/
/*
static s32 goodix_tool_write(struct file *file, const char __user *userbuf,
                        size_t count, loff_t *ppos)
*///modify stephen
						
static ssize_t goodix_tool_write(struct file *file, const char __user *userbuf,
                        size_t count, loff_t *ppos)
{
    s32 ret = 0;
    ...
    if (ret) {
        dev_err(&client->dev, "copy_from_user failed.");
        ret = -EACCES;
        goto exit;
    }

    dev_dbg(&client->dev, "wr:0x%02x, flag:0x%02x, flag addr:0x%02x%02x, flag val:0x%02x,"
            "flag rel:0x%02x, circle:%d, times:%d, retry:%d, delay:%d,"
    ...
    if (cmd_head.data_len > (data_length - GTP_ADDR_LENGTH)) {
        dev_err(&client->dev, "data len %d > data buff %d, rejected!\n",
            cmd_head.data_len, (data_length - GTP_ADDR_LENGTH));
        ret = -EINVAL;
        goto exit;
    }
    if (cmd_head.addr_len > GTP_ADDR_LENGTH) {
        dev_err(&client->dev, "addr len %d > data buff %d, rejected!\n",
    ...
    if (cmd_head.wr == GTP_RW_WRITE) {
        ...
        if (cmd_head.flag == GTP_NEED_FLAG) {
            if (comfirm(client) == FAIL) {
                dev_err(&client->dev, "Comfirm fail!");
                ret = -EINVAL;
                goto exit;
            }
        ...
        if (tool_i2c_write(
        client ,&cmd_head.data[GTP_ADDR_LENGTH - cmd_head.addr_len],
        ...
        if (cmd_head.data_len > sizeof(ic_type)) {
            dev_err(&client->dev,
                ...
                (int)cmd_head.data_len, (int)sizeof(ic_type));//stephen
            ret = -EINVAL;
            goto exit;
        }
        ...
        /* Enter update mode! */
        if (gup_enter_update_mode(client) == FAIL) {
            ret = -EBUSY;
            goto exit;
        }
    } else if (cmd_head.wr == GTP_RW_LEAVE_UPDATE_MODE) {
        /* Leave update mode! */
        gup_leave_update_mode(client);
        ...
        if (gup_update_proc(((void *)cmd_head.data), client) == FAIL) {
            ret = -EBUSY;
            goto exit;
        }
    }
                    ...
                    size_t count, loff_t *ppos)
{
    ...
    mutex_lock(&lock);
    if (cmd_head.wr & 0x1) {
        dev_err(&client->dev, "command head wrong\n");
        ret = -EINVAL;
        goto exit;
    }
    ...
    case GTP_RW_READ:
        ...
        if (data_len <= 0 || (data_len > data_length)) {
            dev_err(&client->dev, "Invalid data length %d\n",
                data_len);
            ret = -EINVAL;
            goto exit;
        }
        ...
        if (tool_i2c_read(client, cmd_head.data, data_len) <= 0) {
...
static int goodix_tool_open(struct inode *inode, struct file *file)
{
    if (inode)
...
{
    u8 i;
    struct proc_dir_entry *goodix_proc_entry = NULL;
    struct goodix_ts_data *ts = i2c_get_clientdata(client);
    ...
    mutex_init(&lock);
    tool_set_proc_name(ts);
	//modify stephen
	
    goodix_proc_entry = proc_create_data(ts->procname,
