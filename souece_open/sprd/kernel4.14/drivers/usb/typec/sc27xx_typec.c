...
#ifdef CONFIG_GHT_ADD_TYPEC_ARRT
#define SC27XX_TYPEC_STATUS					0x1c
#define SC27XX_TYPEC_CC_CHANNEL_MASK		BIT(4)
#define SC27XX_TYPEC_CC1_CHANNEL			1
#define SC27XX_TYPEC_CC2_CHANNEL			2
#define SC27XX_TYPEC_CC_CHANNEL_DET_MASK	GENMASK(8, 5)
#define SC27XX_TYPEC_CC1_CHANNEL_DET		0x20
#define SC27XX_TYPEC_CC1_CHANNEL_HIGH		0x60
#define SC27XX_TYPEC_CC2_CHANNEL_DET		0x80
#define SC27XX_TYPEC_CC2_CHANNEL_HIGH		0x180
#define SC27XX_TYPEC_CC1_DET_CC2_HIGH		(SC27XX_TYPEC_CC1_CHANNEL_DET | \
							SC27XX_TYPEC_CC2_CHANNEL_HIGH)
#define SC27XX_TYPEC_CC2_DET_CC1_HIGH		(SC27XX_TYPEC_CC2_CHANNEL_DET | \
								SC27XX_TYPEC_CC1_CHANNEL_HIGH)
#define SC27XX_TYPEC_CC_CHANNEL_DET_REG		SC27XX_TYPEC_STATUS
#endif
			...
			struct device_attribute *attr, char *buf)
{
	...
	regmap_read(sc->regmap, sc->base + SC27XX_TYPEC_STATUS, &var);
	cc_state = var & SC27XX_TYPEC_CC_CHANNEL_MASK ?
			SC27XX_TYPEC_CC1_CHANNEL : SC27XX_TYPEC_CC2_CHANNEL;

	regmap_read(sc->regmap, sc->base + SC27XX_TYPEC_CC_CHANNEL_DET_REG, &var);
	cc_dbg_state = var & SC27XX_TYPEC_CC_CHANNEL_DET_MASK;

	switch (cc_state) {
		case SC27XX_TYPEC_CC1_CHANNEL:
			if (cc_dbg_state == SC27XX_TYPEC_CC1_CHANNEL_DET)
				state = 0;
			else if (cc_dbg_state == SC27XX_TYPEC_CC1_DET_CC2_HIGH)
				state = 2;
			else
				state = 4;
			break;

		case SC27XX_TYPEC_CC2_CHANNEL:
			if (cc_dbg_state == SC27XX_TYPEC_CC2_CHANNEL_DET)
				state = 1;
			else if (cc_dbg_state == SC27XX_TYPEC_CC2_DET_CC1_HIGH)
				state = 3;
			else
				state = 4;
			break;
	...
	dev_info(sc->dev, "cc status=%x\n", var);

	return sprintf(buf, "%x\n", state);
}
static DEVICE_ATTR_RO(bbat_ccx_state);

static ssize_t ft_data_role_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	struct sc27xx_typec *sc = dev_get_drvdata(dev);
	int role;
...
static DEVICE_ATTR_RO(ft_data_role);
#endif
...
#ifdef CONFIG_GHT_ADD_TYPEC_ARRT
	if (device_create_file(&pdev->dev, &dev_attr_bbat_ccx_state))
		dev_warn(&pdev->dev, "could not create device file for cc orientation!\n");
	if (device_create_file(&pdev->dev, &dev_attr_ft_data_role))
		dev_warn(&pdev->dev, "could not create device file for data role!\n");
#endif
	return 0;
