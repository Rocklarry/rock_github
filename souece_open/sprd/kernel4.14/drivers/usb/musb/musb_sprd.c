...
#ifdef CONFIG_GHT_FEATURE_USB_WAKEUP_EN
...
struct sprd_glue {
...
#ifndef CONFIG_GHT_ENABLE_GPIO_OTG_POWER
	...
	//Modify by hangzhou.zhang 20200616 begin
	struct extcon_dev		*dfp_edev;
	//Modify by hangzhou.zhang 20200616 end
...
#ifdef CONFIG_GHT_ENABLE_GPIO_OTG_POWER
static int musb_sprd_otg_init(struct sprd_glue *glue)
{
	int ret;

	ret = devm_gpio_request(glue->dev, glue->gpio_otg, "musb otg ext power detect");
	if (ret < 0) {
...
#ifdef CONFIG_GHT_FEATURE_USB_WAKEUP_EN
	if (glue->input_dev) {
		input_report_key(glue->input_dev, KEY_WAKEUP, 1);
		input_sync(glue->input_dev);
		input_report_key(glue->input_dev, KEY_WAKEUP, 0);
		input_sync(glue->input_dev);
	}
#endif
...
//Modify by hangzhou.zhang 20200616 begin
static int musb_sprd_dfp_notifier(struct notifier_block *nb,
				unsigned long event, void *data)
{
	struct sprd_glue *glue = container_of(nb, struct sprd_glue, dfp_nb);
	unsigned long flags;
	...
	if (event) {
		...
		if (glue->vbus_active == 1 || glue->dr_mode == USB_DR_MODE_PERIPHERAL) {
			spin_unlock_irqrestore(&glue->lock, flags);
			dev_info(glue->dev,
				"ignore host connection detected from TypeC.\n");
			return 0;
		}
		...
		spin_unlock_irqrestore(&glue->lock, flags);
		dev_info(glue->dev,
			"host connection detected from TypeC.\n");
	} else {
		spin_lock_irqsave(&glue->lock, flags);
		if (glue->vbus_active == 0 || glue->dr_mode == USB_DR_MODE_PERIPHERAL) {
			spin_unlock_irqrestore(&glue->lock, flags);
			dev_info(glue->dev,
				"ignore host disconnect detected from TypeC.\n");
			return 0;
		}
		...
		spin_unlock_irqrestore(&glue->lock, flags);
		dev_info(glue->dev,
			"host disconnect detected from TypeC.\n");
	}
#ifdef CONFIG_GHT_FEATURE_USB_WAKEUP_EN
	if (glue->input_dev) {
		input_report_key(glue->input_dev, KEY_WAKEUP, 1);
...
//Modify by hangzhou.zhang 20200616 begin
	struct extcon_dev *dfp_ext = glue->dfp_edev ? glue->dfp_edev : glue->edev;
//Modify by hangzhou.zhang 20200616 end
	...
	if (extcon_get_state(id_ext, EXTCON_USB_HOST) == true) {
		...
		queue_work(system_unbound_wq, &glue->work);
//Modify by hangzhou.zhang 20200616 begin
	} else if (extcon_get_state(dfp_ext, EXTCON_USB_HOST) == true) {
		if (glue->vbus_active == 1) {
			spin_unlock_irqrestore(&glue->lock, flags);
			dev_info(glue->dev,
				"ignore device connection detected from ID GPIO.\n");
...
static int musb_sprd_probe(struct platform_device *pdev)
{
	...
	if (pdata.mode == MUSB_PORT_MODE_HOST ||
		...
		if (gpio_is_valid(glue->gpio_otg)) {
			dev_info(dev, "get the gpio to detect otg");
		} else {
...
//Modify by hangzhou.zhang 20200616 begin
		glue->dfp_edev = extcon_get_edev_by_phandle(glue->dev, 2);
		if (IS_ERR(glue->dfp_edev)) {
			glue->dfp_edev = NULL;
			dev_info(dev, "No typec extcon device.\n");
		}

		glue->dfp_nb.notifier_call = musb_sprd_dfp_notifier;
		if (glue->dfp_edev)
			ret = extcon_register_notifier(glue->dfp_edev,
						       EXTCON_USB_HOST,
						       &glue->dfp_nb);
		else
			ret = extcon_register_notifier(glue->edev,
...
#ifdef CONFIG_GHT_FEATURE_USB_WAKEUP_EN
	glue->input_dev = input_allocate_device();
	if(!glue->input_dev) {
		dev_err(glue->dev, "input device register failed.\n");
	} else {
		glue->input_dev->name = "gpio_usbdetect";
		glue->input_dev->phys = "input/usbdetect";
		glue->input_dev->id.bustype = BUS_USB;
		set_bit(EV_KEY, glue->input_dev->evbit);
		set_bit(EV_SYN, glue->input_dev->evbit);
		set_bit(KEY_WAKEUP, glue->input_dev->keybit);
		ret = input_register_device(glue->input_dev);
		if(ret) {
			input_free_device(glue->input_dev);
			glue->input_dev = NULL;
		}
	}
#endif
...
#ifdef CONFIG_GHT_FEATURE_USB_WAKEUP_EN
	if (glue->input_dev) {
		input_unregister_device(glue->input_dev);
		input_free_device(glue->input_dev);
		glue->input_dev = NULL;
	}
#endif
