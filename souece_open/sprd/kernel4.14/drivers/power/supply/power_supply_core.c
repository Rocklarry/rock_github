...
/*
 * Get the y value of x according to the coordinates (x0, y0) and (x1, y1)
 */
				  ...
				  struct power_supply_battery_info *info)
{
	...
	for (index = 0; index < POWER_SUPPLY_OCV_TEMP_MAX; index++) {
		...
		info->rint_temp[index]       = -EINVAL;
		info->rint_cap_temp[index]   = NULL;
	}

	for (index = 0; index < POWER_SUPPLY_CAP_TEMP_MAX; index++) {
		info->rint_cap[index]        = -EINVAL;
	}
	...
	// Add battery internal resistance parameters at different temperatures
	len = of_property_count_u32_elems(battery_np, "rint-temp-celsius");
	if (len < 0 && len != -EINVAL) {
		return len;
	} else if (len > POWER_SUPPLY_OCV_TEMP_MAX) {
		dev_err(&psy->dev, "Too many temperature values\n");
		...
		of_property_read_u32_array(battery_np, "rint-temp-celsius",
						info->rint_temp, len);
		info->rint_temp_size = len;
	}

	len1 = of_property_count_u32_elems(battery_np, "rint-capacity-celsius");
	if (len1 < 0 && len1 != -EINVAL) {
		return len1;
	} else if (len1 > POWER_SUPPLY_CAP_TEMP_MAX) {
		dev_err(&psy->dev, "Too many capacity values\n");
		return -EINVAL;
	} else if (len1 > 0) {
		of_property_read_u32_array(battery_np, "rint-capacity-celsius",
						info->rint_cap, len1);
		info->rint_cap_size = len1;
	}

	for (index = 0; index < len; index++) {
		info->rint_cap_temp[index] = devm_kzalloc(&psy->dev,
						len1 * sizeof(int),
						GFP_KERNEL);
		if (!info->rint_cap_temp[index]) {
			power_supply_put_battery_info(psy, info);
			return -ENOMEM;
		}
	}

	list = of_get_property(battery_np, "rint-capacity-table", &size);
	if (!list || !size) {
		dev_err(&psy->dev, "failed to get rint-capacity-table!\n");
		power_supply_put_battery_info(psy, info);
		return -EINVAL;
	}

	size = size / (len * sizeof(__be32));
	if (size != len1) {
		dev_err(&psy->dev, "the number of rint does not match!\n");
		power_supply_put_battery_info(psy, info);
		return -EINVAL;
	}
	...
	for (index = 0; index < len1; index++) {
		int i;
		for (i = 0; i < len; i++)
			info->rint_cap_temp[i][index] = be32_to_cpu(*list++);
	}
				   ...
				   struct power_supply_battery_info *info)
{
	...
	for (i = 0; i < POWER_SUPPLY_OCV_TEMP_MAX; i++) {
        ...
        for (i = 0; i < POWER_SUPPLY_OCV_TEMP_MAX; i++) {
                if (info->rint_cap_temp[i])
                        devm_kfree(&psy->dev, info->rint_cap_temp[i]);
        }
...
/*
 * Get the battery capacity based on battery temperature and battery's current OCV
 */
int power_supply_find_soc(struct power_supply_battery_info *info,
				int batt_temp, int ocv)
{
	int i, j, pcj, pcj_minus_one, pc;
	int rows = info->ocv_table_size[0];
	int cols = info->ocv_temp_size;

	if (batt_temp < info->ocv_temp[0]) {
		pr_debug("batt_temp %d < known temp range\n", batt_temp);
		batt_temp = info->ocv_temp[0];
	}

	if (batt_temp > info->ocv_temp[cols - 1]) {
		pr_debug("batt_temp %d > known temp range\n", batt_temp);
		batt_temp = info->ocv_temp[cols - 1];
	}

	for (j = 0; j < cols; j++)
		if (batt_temp <= info->ocv_temp[j])
			break;
	if (batt_temp == info->ocv_temp[j]) {
		/* found an exact match for temp in the table */
		if (ocv >= (info->ocv_table[j][0]).ocv)
			return (info->ocv_table[j][0]).capacity;
		if (ocv <= (info->ocv_table[j][rows - 1]).ocv)
			return (info->ocv_table[j][rows - 1]).capacity;
		for (i = 0; i < rows; i++) {
			if (ocv >= (info->ocv_table[j][i]).ocv) {
				if (ocv == (info->ocv_table[j][i]).ocv)
					return (info->ocv_table[j][i]).capacity;
				pc = linear_interpolate(
					(info->ocv_table[j][i]).capacity,
					(info->ocv_table[j][i]).ocv,
					(info->ocv_table[j][i-1]).capacity,
					(info->ocv_table[j][i-1]).ocv,
					ocv);
				return pc;
			}
		}
	...
	if (ocv >= (info->ocv_table[j][0]).ocv)
		return (info->ocv_table[j][0]).capacity;
	if (ocv <= (info->ocv_table[j-1][rows - 1]).ocv)
		return (info->ocv_table[j-1][rows - 1]).capacity;

	pcj_minus_one = 0;
	pcj = 0;
	for (i = 0; i < rows-1; i++) {
		if (pcj == 0
			&& is_between((info->ocv_table[j][i]).ocv,
		...
		if (pcj_minus_one == 0
			&& is_between((info->ocv_table[j-1][i]).ocv,
				(info->ocv_table[j-1][i+1]).ocv, ocv)) {
			pcj_minus_one = linear_interpolate(
				(info->ocv_table[j-1][i]).capacity,
				(info->ocv_table[j-1][i]).ocv,
				(info->ocv_table[j-1][i+1]).capacity,
				(info->ocv_table[j-1][i+1]).ocv,
				ocv);
		}
...
EXPORT_SYMBOL_GPL(power_supply_find_soc);


/*
 * Get battery internal resistance based on battery temperature and specified soc
 */
int power_supply_find_rint(struct power_supply_battery_info *info,
						int row_entry,
						int pc)
{
	...
	rows = info->rint_cap_size;
	cols = info->rint_temp_size;
	if (pc > info->rint_cap[0]) {
		pr_debug("pc %d greater than known pc ranges for sfd\n", pc);
		row1 = 0;
		row2 = 0;
	} else if (pc < info->rint_cap[rows - 1]) {
		pr_debug("pc %d less than known pc ranges for sf\n", pc);
		row1 = rows - 1;
		row2 = rows - 1;
	} else {
		for (i = 0; i < rows; i++) {
			if (pc == info->rint_cap[i]) {
				row1 = i;
				row2 = i;
				break;
			}
	...
	if (row_entry < info->rint_temp[0])
		row_entry = info->rint_temp[0];
	if (row_entry > info->rint_temp[cols - 1])
		row_entry = info->rint_temp[cols - 1];

	for (i = 0; i < cols; i++)
		if (row_entry <= info->rint_temp[i])
			break;
	if (row_entry == info->rint_temp[i]) {
		scalefactor = linear_interpolate(
				info->rint_cap_temp[i][row1],
				info->rint_cap[row1],
				info->rint_cap_temp[i][row2],
				info->rint_cap[row2],
				pc);
		return scalefactor;
	}
...
EXPORT_SYMBOL_GPL(power_supply_find_rint);
