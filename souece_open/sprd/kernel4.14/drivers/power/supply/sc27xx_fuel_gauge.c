...
static int vol_mode_init_cap = 0;
#endif
...
#ifdef CONFIG_GHT_CREATE_CHG_CUR_ATTR
	struct iio_channel *charger_cur;
#endif
	bool bat_present;
	int internal_resist;
	int total_cap;
...
static int sc27xx_fgu_get_boot_capacity(struct sc27xx_fgu_data *data, int *cap)
{
	...
	*cap = power_supply_ocv2cap_simple(data->cap_table, data->table_len,
	...
	if (!is_first_poweron) {
		...
		if (ret)
			return ret;
//modified by hangzhou.zhang for mantis bug-0047655 20200527 begin
#if 0
...
#endif
//modified by hangzhou.zhang for mantis bug-0047655 20200527 end
			*cap = saved_cap;
	}
...
static int sc27xx_fgu_get_capacity(struct sc27xx_fgu_data *data, int *cap)
{
	...
	*cap = power_supply_ocv2cap_simple(data->cap_table, data->table_len, ocv);
	pr_err("Check OCV table vol:%d, cap:%d\n", ocv, *cap);
#endif
...
#ifdef CONFIG_GHT_CREATE_CHG_CUR_ATTR
static int sc27xx_fgu_get_charger_cur(struct sc27xx_fgu_data *data, int *cur)
{
        int ret, temp_cur;
        
	ret = iio_read_channel_processed(data->charger_cur, &temp_cur);
        if (ret < 0)
		return ret;
		...
		struct device_attribute *attr, char *buf)
{
	...
	psy = power_supply_get_by_name("sc2721_charger");
	if (!psy) {
		dev_err(data->dev, "Cannot find power supply sc2721_charger!\n");
		return -EINVAL;
	}

	ret = sc27xx_fgu_get_charger_cur(data, &cur);
	if(ret < 0) {
		dev_err(data->dev, "Cannot get adc channel current value!\n");
		return -EINVAL;
	}
	...
	if (ret == 0) {
		cc_cur = val.intval / 1000;
	} else {
		dev_err(data->dev, "Cannot get the cc value!\n");
		return -EINVAL;
	}

	dev_err(data->dev, "adc cur is %d, cc_cur is %d!\n", cur, cc_cur);
	cur = (cur * 10) / 12;
	cur = cur * cc_cur;
...
#ifdef CONFIG_GHT_FEATURE_ADD_TBAT_VOL
static ssize_t bat_temp_vol_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	struct sc27xx_fgu_data *data = dev_get_drvdata(dev);
	int ret, vol;
...
static DEVICE_ATTR_RO(bat_temp_vol);
#endif
...
#ifdef CONFIG_GHT_ENABLE_ONLY_VOL_MODE
		pr_info("The raw initial ui cap is %d at vol mode!\n", value);
		if (vol_mode_init_cap > 0) {
			pr_info("The fixed initial ui cap is %d at vol mode!\n", vol_mode_init_cap);
			value = vol_mode_init_cap;
			vol_mode_init_cap = 0;
		}
#endif
		if (ret)
...
#ifdef CONFIG_GHT_ENABLE_ONLY_VOL_MODE
	vol_mode_init_cap = data->init_cap;
#endif
...
#ifdef CONFIG_GHT_CREATE_CHG_CUR_ATTR
	data->charger_cur = devm_iio_channel_get(&pdev->dev, "charger-cur");
	if (IS_ERR(data->charger_cur)) {
		dev_err(&pdev->dev, "failed to get charge IIO channel\n");
		data->charger_cur = NULL;
	}
#endif
...
#ifdef CONFIG_GHT_CREATE_CHG_CUR_ATTR
	if (device_create_file(&pdev->dev, &dev_attr_charger_current))
		dev_warn(&pdev->dev, "could not create sysfs file for charger current!\n");
#endif
#ifdef CONFIG_GHT_FEATURE_ADD_TBAT_VOL
	if (device_create_file(&pdev->dev, &dev_attr_bat_temp_vol))
		dev_warn(&pdev->dev, "could not create sysfs file for bat temp vol!\n");
#endif
