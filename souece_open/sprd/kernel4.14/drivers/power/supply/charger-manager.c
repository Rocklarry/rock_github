...
#ifdef CONFIG_GHT_ENABLE_ONLY_VOL_MODE
...
#ifdef CONFIG_GHT_ENABLE_ONLY_VOL_MODE
#define CM_POLL_INTERVAL_FAST		1000
#define FAST_CAP_ONE_TIME		30
#define ALARM_SOC			20
static int enable_fast_polling = 0;
static int enable_charging_flag = 0;
static int enable_full = 0;
static int last_cap_one_time = 0;
...
static int get_batt_cap(struct charger_manager *cm, int *cap)
{
	...
	*cap = val.intval;
#ifdef CONFIG_GHT_ENABLE_ONLY_VOL_MODE
	if ((*cap == 0) && (is_ext_pwr_online(cm)))
		*cap = 1;
	dev_info(cm->dev, "Fixed battery cap is %d\n", *cap);
#endif
	return 0;
}
...
#ifndef CONFIG_GHT_ENABLE_ONLY_VOL_MODE
/**
...
static bool is_full_charged(struct charger_manager *cm) {
	...
	if (desc->fullbatt_uV > 0) {
		for (i = 0; i< FULL_CHACK_NUM; i++) {
			ret = get_batt_uV(cm, &uV);
			if (ret) {
				desc->trigger_cnt = 0;
				dev_err(cm->dev, "Cannot read vbat!\n");
				return false;
			}
		...
		if (desc->trigger_cnt >= FULL_CHACK_NUM) {
			if (cm->desc->cap >= 99)
				is_full = true;
			else
				is_full = false;
...
#ifdef CONFIG_GHT_ENABLE_ONLY_VOL_MODE
			if (enable_fast_polling)
				min = CM_POLL_INTERVAL_FAST;
#endif
		}
	}
	if (keep_polling)
		pr_err("Keeping polling is enable, polling time is %lu!\n", min);
	else
		pr_err("Keeping polling is disabled!\n");
...
#ifdef CONFIG_GHT_ENABLE_ONLY_VOL_MODE
	dev_info(cm->dev, "start _cm_monitor!\n");
#endif
	/* Feed the charger watchdog if necessary */
	ret = cm_feed_watchdog(cm);
	if (ret) {
#ifdef CONFIG_GHT_ENABLE_ONLY_VOL_MODE
		dev_info(cm->dev, "Failed to feed charger watchdog\n");
#endif
...
#ifdef CONFIG_GHT_ENABLE_ONLY_VOL_MODE
	dev_info(cm->dev, "print charging state is %d, charger enable is %d!\n",
			cm->charging_status, cm->charger_enabled);
	if (cm->charging_status == 0) {
		if (cm->charger_enabled) {
			try_charger_enable(cm, false);
			enable_fast_polling = 1;
			enable_charging_flag = 1;
			dev_info(cm->dev, "only_vol_mode disable charging!\n");
			return true;
		} else if (enable_fast_polling) {
			enable_fast_polling = 0;
			dev_info(cm->dev, "only_vol_mode get capacity in discharging mode!\n");
			cm_batt_works(&cm->cap_update_work.work);
			dev_info(cm->dev, "only_vol_mode enable charging!\n");
			try_charger_enable(cm, true);
			enable_charging_flag = 0;
			msleep(20);
		}
	} else {
		dev_info(cm->dev, "only_vol_mode get capacity in emergency stop!\n");
		enable_charging_flag = 0;
		cm_batt_works(&cm->cap_update_work.work);
		enable_full = 0;
	}
...
#ifdef CONFIG_GHT_ENABLE_ONLY_VOL_MODE
	} else if (!cm->emergency_stop && enable_full && cm->charger_enabled) {
...
#ifdef CONFIG_GHT_ENABLE_ONLY_VOL_MODE
			if (enable_full || cm->desc->force_set_full)
#else
			if (is_full_charged(cm) || cm->desc->force_set_full)
#endif
				val->intval = POWER_SUPPLY_STATUS_FULL;
#ifdef CONFIG_GHT_ENABLE_ONLY_VOL_MODE
			else if (cm->charging_status == 0)
				val->intval = POWER_SUPPLY_STATUS_CHARGING;
#endif
			else
				val->intval = POWER_SUPPLY_STATUS_NOT_CHARGING;
...
#ifdef CONFIG_GHT_ENABLE_ONLY_VOL_MODE
	last_cap_one_time = desc->cap_one_time;
#endif
...
static void cm_batt_works(struct work_struct *work)
{
...
#ifdef CONFIG_GHT_ENABLE_ONLY_VOL_MODE
	if (is_charging(cm)) {
		dev_info(cm->dev, "only-vol-mode cannot update capacity in charging mode!\n");
	...
	if ((enable_charging_flag == 1) && (enable_fast_polling == 1)) {
		dev_info(cm->dev, "the timer time of only-vol-mode does not expire!\n");
		queue_delayed_work(system_power_efficient_wq,
					&cm->cap_update_work,
					CM_CAP_CYCLE_TRACK_TIME * HZ);
		return;
	...
	if (cm->desc->cap < ALARM_SOC)
		cm->desc->cap_one_time = FAST_CAP_ONE_TIME;
	else
		cm->desc->cap_one_time = last_cap_one_time;
	dev_info(cm->dev, "the one time is %ds!\n", cm->desc->cap_one_time);
#endif
...
#else
			{
				if (is_full_charged(cm) && (cm->desc->cap == 99)) {
					fuel_cap = 100;
					enable_full = 1;
				} else {
					fuel_cap = 99;
