...
{
	struct sprd_uart_port *sp = container_of(port, struct sprd_uart_port, port);
	unsigned int ien, iclr;

	if (port->line == SPRD_CM4_UART2AP) {
		sp->tx_start = false;
		return;
	}
...
void sprd_cm4_uart_rx(int event, void *data)
{
...
static void sprd_cm4_uart_tx(struct kthread_work *work)
{
	...
	{
		if (uart_circ_empty(xmit)  || !sp->tx_start) {
			spin_unlock(&port->lock);
			return;
		}
	}else{
		if (uart_circ_empty(xmit)  || !sp->tx_start || uart_tx_stopped(port)) {
			spin_unlock(&port->lock);
			return;
		}
	}
...
static int sprd_cm4_uart_init(struct sprd_uart_port *sp)
{
	...
	if (ret) {
		dev_emerg(sp->port.dev, "init sbuf fail\n");
		return ret;
	}
	...
	if (ret) {
		dev_emerg(sp->port.dev, "register sbuf notifier fail\n");
		return ret;
	}
...
static int sprd_clk_init(struct uart_port *uport)
{
	...
	if (IS_ERR(clk_uart)) {
		dev_emerg(uport->dev, "uart%d can't get uart clock\n",
			 uport->line);
		clk_uart = NULL;
	}
	...
	if (IS_ERR(clk_parent)) {
		dev_emerg(uport->dev, "uart%d can't get source clock\n",
			 uport->line);
		clk_parent = NULL;
	}
	...
	if (IS_ERR(u->clk)) {
		...
		dev_emerg(uport->dev, "uart%d can't get enable clock\n",
			uport->line);
...
static int sprd_probe(struct platform_device *pdev)
{
	...
	if (ret){
		pr_emerg("uart%d clk init failed\r\n",index);
		return ret;
	}
	...
	if (IS_ERR(up->membase)){
		pr_emerg("uart%d membase failed\r\n",index);
	...
	if (irq < 0) {
		dev_emerg(&pdev->dev, "not provide irq resource: %d\n", irq);
		return irq;
	}
	up->irq = irq;
	...
	if (!sprd_ports_num) {
		...
		if (ret < 0) {
			dev_emerg(&pdev->dev, "Failed to register SPRD-UART driver\n");
			return ret;
		}
	}
	sprd_ports_num++;

	if (index == SPRD_CM4_UART2AP) {

		sprd_port[index]->aon_apb =
				syscon_regmap_lookup_by_phandle(pdev->dev.of_node,
								"sprd,aon-apb");
		if (IS_ERR(sprd_port[index]->aon_apb))	{
			dev_emerg(&pdev->dev,"not find aon apb config\n");
			return	-ENOMEM;
		}

		ret = sprd_cm4_uart_init(sprd_port[index]);
		if (ret) {
			pr_emerg("cm4 uart init failed\r\n");
			sprd_remove(pdev);
			return ret;
		}
	}
