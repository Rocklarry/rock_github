...
#ifdef CONFIG_GHT_CREATE_VRTC_ATTR
#define GHT_CTL_BASE_ANA_GLB 		0x0C00
#define GHT_ANA_REG_GLB_LDO_RTC_CTRL	(GHT_CTL_BASE_ANA_GLB + 0x0178)
#endif
...
#ifdef CONFIG_GHT_CREATE_VRTC_ATTR
static ssize_t rtc_voltage_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	...
	regmap_read(rtc->regmap, GHT_ANA_REG_GLB_LDO_RTC_CTRL, &val);
	
	return sprintf(buf, "%d\n", val);
}
static DEVICE_ATTR_RO(rtc_voltage);
#endif
...
#ifdef CONFIG_GHT_CREATE_VRTC_ATTR
	if (device_create_file(&pdev->dev, &dev_attr_rtc_voltage))
		dev_warn(&pdev->dev, "could not create sysfs file for rtc volatage!\n");
#endif
