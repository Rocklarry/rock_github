...
int msm8x16_wcd_digital_mute(struct snd_soc_dai *dai, int mute)
{
	...
	//printk ("===rrd=== %s: Digital Mute val = %d\n", __func__, mute);
    if(!mute){
        //gpio_set_value_cansleep(8, 0);//打开PA
        //printk("%s ===rrd===  ap open",__func__);

		pr_info("*************audio gpio 132 open ************\n");
        ...
        //gpio_set_value_cansleep(8, 1);//关闭PA
        //printk("%s ===rrd===  ap close",__func__);

		pr_info("*************audio gpio 132 close ************\n");
		gpio_set_value_cansleep(132, 0);
