...
static int quin_tx_sample_rate = SAMPLING_RATE_48KHZ;
...
static int msm8952_wsa_switch_event(struct snd_soc_dapm_widget *w,
			      struct snd_kcontrol *kcontrol, int event);
static int fpga_reset(struct snd_soc_card *card);

extern int has_three_codec(void);
...
static char const *mi2s_rx_sample_rate_text[] = {"KHZ_48",
					"KHZ_96", "KHZ_192"};
static char const *quin_tx_sample_rate_text[] = {"KHZ_48",
					"KHZ_96", "KHZ_192"};
...
extern int msm_fpga_reset_en(enum pinctrl_client client,int enable);	

int is_fpga_reset_gpio_support(struct platform_device *pdev,
			struct msm8916_asoc_mach_data *pdata)
{
	const char *fpga_reset = "fibocom,fpga-reset-gpio";
	int ret=0;
	pr_err("%s:Enter\n", __func__);

	pdata->fgpa_reset_gpio = of_get_named_gpio(pdev->dev.of_node,
				fpga_reset, 0);

	if (pdata->fgpa_reset_gpio < 0) {
		dev_err(&pdev->dev,
			"%s: missing %s in dt node\n", __func__, fpga_reset);
	} else {
		if (!gpio_is_valid(pdata->fgpa_reset_gpio)) {
			pr_err("%s: Invalid fpga reset gpio: %d",
				__func__, pdata->fgpa_reset_gpio);
			return -EINVAL;
		}
	}
	ret =msm_fpga_reset_en(CLIENT_WCD_INT,1);
		if (ret) {
			pr_err("%s: gpio set cannot be de-activated %s\n",
					__func__, "ext_spk_gpio");
			return ret;
		}
	    gpio_set_value_cansleep(pdata->fgpa_reset_gpio, 1);
	ret =msm_fpga_reset_en(CLIENT_WCD_INT,0);
	return 0;
}
...
static int fpga_reset(struct snd_soc_card *card)
{
	...
	if (!gpio_is_valid(pdata->spk_ext_pa_gpio)) {
	   ...
	   pr_err("zj fpga_reset\n");
		...
		//gpio_set_value_cansleep(pdata->fgpa_reset_gpio, 1);
		gpio_set_value_cansleep(pdata->fgpa_reset_gpio, 0);
		msleep(1);
		gpio_set_value_cansleep(pdata->fgpa_reset_gpio, 1);
		ret =msm_fpga_reset_en(CLIENT_WCD_INT,0);
		if (ret) {
			...
			pr_err("zj fpga_reset  end \n");

	return 0;
}


extern int msm_gpioset_ex_pa_en(enum pinctrl_client client,int enable);//liyongfeng add for ext-pa_enable
extern int msm_gpioset_ex_pa_vdd(enum pinctrl_client client,int enable);

int is_ext_spk_gpio_support(struct platform_device *pdev,
			struct msm8916_asoc_mach_data *pdata)
{
	const char *spk_ext_pa = "qcom,msm-spk-ext-pa";
	const char *spk_ext_pa_vdd = "qcom,msm-spk-ext-pa_vdd";
	int ret=0;
...
//	set default  value high
	ret =msm_gpioset_ex_pa_en(CLIENT_WCD_INT,1);
	if (ret) {
		pr_err("%s: gpio set cannot be de-activated %s\n",
				__func__, "ext_spk_gpio");
		return ret;
    ...
    //printk("%s: ===rrd=== open pa \n", __func__);

	ret =msm_gpioset_ex_pa_en(CLIENT_WCD_INT,0);//liyongfeng add for ex pa enable
	if (ret) {
		pr_err("%s: gpio set cannot be de-activated %s\n",
				__func__, "ext_spk_gpio");
		return ret;
...
//pa vdd en  control
	pdata->spk_ext_pa_gpio_vdd = of_get_named_gpio(pdev->dev.of_node,
				spk_ext_pa_vdd, 0);
	if (pdata->spk_ext_pa_gpio_vdd < 0) {
		dev_dbg(&pdev->dev,
			"%s: missing %s in dt node\n", __func__, spk_ext_pa_vdd);
	}
	ret =msm_gpioset_ex_pa_vdd(CLIENT_WCD_INT,1);
	if (ret) {
		pr_err("%s: gpio set cannot be de-activated %s\n",
				__func__, "ext_spk_gpio_vdd");
		return ret;
	}
	gpio_set_value_cansleep(pdata->spk_ext_pa_gpio_vdd, 0);

	ret =msm_gpioset_ex_pa_vdd(CLIENT_WCD_INT,0);//liyongfeng add for ex pa enable
	if (ret) {
		pr_err("%s: gpio set cannot be de-activated %s\n",
				__func__, "ext_spk_gpio_vdd");
		return ret;
...
static int enable_spk_ext_pa(struct snd_soc_codec *codec, int enable)
{
	...
	gpio_set_value_cansleep(pdata->spk_ext_pa_gpio_vdd, enable);
	ret =msm_gpioset_ex_pa_vdd(CLIENT_WCD_INT,0);
		if (ret) {
			pr_err("%s: gpio set cannot be de-activated %s\n",
					__func__, "ext_spk_gpio");
			return ret;
	...
	ret =msm_gpioset_ex_pa_en(CLIENT_WCD_INT,0);
		if (ret) {
			pr_err("%s: gpio set cannot be de-activated %s\n",
					__func__, "ext_spk_gpio");
			return ret;
...
static int msm_quin_tx_hw_params_fixup(struct snd_soc_pcm_runtime *rtd,
				struct snd_pcm_hw_params *params)
{
	struct snd_interval *rate = hw_param_interval(params,
					SNDRV_PCM_HW_PARAM_RATE);
	pr_debug("%s()\n", __func__);
	rate->min = rate->max = quin_tx_sample_rate;
...
static int msm_mi2s_snd_hw_params(struct snd_pcm_substream *substream,
			     struct snd_pcm_hw_params *params)
{
	pr_debug("%s(): substream = %s  stream = %d substream->pcm->id:(%s)\n", __func__,
		 substream->name, substream->stream,substream->pcm->id);

	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
		param_set_mask(params, SNDRV_PCM_HW_PARAM_FORMAT,
			       mi2s_rx_bit_format);
	else{
            if(strncmp(substream->pcm->id,"(Quaternary MI2S Capture)",strlen("(Quaternary MI2S Capture)")) !=0){
		param_set_mask(params, SNDRV_PCM_HW_PARAM_FORMAT,
			       SNDRV_PCM_FORMAT_S16_LE);
            }else
                 pr_debug("Quaternary MI2S Capture no setting");
        }
		...
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			...
			if (pdata->afe_clk_ver == AFE_CLK_VERSION_V1) {
				...
				mi2s_tx_clk.clk_id =
					msm8952_get_clk_id(port_id);
                if(strncmp(substream->pcm->id,"(Quaternary MI2S Capture)",strlen("(Quaternary MI2S Capture)")) ==0){
                    pr_debug("Quaternary MI2S Capture setting clk to 3.072MHZ");
				    mi2s_tx_clk.clk_freq_in_hz = Q6AFE_LPASS_IBIT_CLK_3_P072_MHZ;
                }else if (strncmp(substream->pcm->id,"(Quinary MI2S Capture)",strlen("(Quinary MI2S Capture)")) ==0){
                   if(has_three_codec())
			{
			 pr_debug("Quinary MI2S Capture setting clk to 3.072MHZ");
			 mi2s_tx_clk.clk_freq_in_hz = Q6AFE_LPASS_IBIT_CLK_3_P072_MHZ;
			}
		else
			{
			pr_debug("Quinary MI2S Capture setting clk to 1.536MHZ");
			mi2s_tx_clk.clk_freq_in_hz = Q6AFE_LPASS_IBIT_CLK_1_P536_MHZ;

			}
                }else{
                    pr_debug("Default MI2S Capture setting clk to 1.536MHZ");
				    mi2s_tx_clk.clk_freq_in_hz = Q6AFE_LPASS_IBIT_CLK_1_P536_MHZ;
                }
				ret = afe_set_lpass_clock_v2(port_id,
							&mi2s_tx_clk);
...
static int quin_tx_sample_rate_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	int sample_rate_val = 0;

	switch (quin_tx_sample_rate) {
		case SAMPLING_RATE_96KHZ:
			sample_rate_val = 1;
			break;
		case SAMPLING_RATE_192KHZ:
...
static int quin_tx_sample_rate_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	switch (ucontrol->value.integer.value[0]) {
		case 1:
			quin_tx_sample_rate = SAMPLING_RATE_96KHZ;
			break;
		case 2:
			quin_tx_sample_rate = SAMPLING_RATE_192KHZ;
			break;
		case 0:
		default:
			quin_tx_sample_rate = SAMPLING_RATE_48KHZ;
			break;
	}
	pr_debug("%s: mi2s_rx_sample_rate = %d\n", __func__,
			quin_tx_sample_rate);
...
static const struct soc_enum msm_snd_enum[] = {
	...
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(quin_tx_sample_rate_text),
				quin_tx_sample_rate_text),
};
...
static const struct snd_kcontrol_new msm_snd_controls[] = {
	...
	SOC_ENUM_EXT("QUIN_TX SampleRate", msm_snd_enum[7],
			quin_tx_sample_rate_get, quin_tx_sample_rate_put),
};
...
static int msm_mi2s_snd_startup(struct snd_pcm_substream *substream)
{
    ...
    msleep(100);
    //gpio_set_value_cansleep(pdata->spk_ext_pa_gpio, 0);//打开PA
    //printk("%s: ===rrd=== open pa \n", __func__);
...
static void msm_mi2s_snd_shutdown(struct snd_pcm_substream *substream)
{
    ...
    //gpio_set_value_cansleep(pdata->spk_ext_pa_gpio, 1); //关闭pa
    //printk("%s: ===rrd=== close  pa \n", __func__);
}
...
static struct snd_soc_dai_link msm8952_dai[] = {
	...
	
	{
		...
		.codec_name = "nau8540.5-001d",
		.no_pcm = 1,
		.dpcm_capture = 1,
		.be_id = MSM_BACKEND_DAI_QUINARY_MI2S_TX,
		.be_hw_params_fixup = msm_quin_tx_hw_params_fixup,
		.ops = &msm8952_quin_mi2s_be_ops,
		.ignore_suspend = 1,
	},
	{
	        .name = "nau8540codec21",
	        .stream_name = "nau8540codec2",
	        .cpu_dai_name = "snd-soc-dummy-dai",
	        .codec_dai_name = "nau8540_1-hifi",
	        .codec_name = "nau8540_1.5-001c",
	        .no_pcm = 1,
	        .dpcm_capture = 1,
	        .be_id = MSM_BACKEND_DAI_NAU8543_1_TX,
	        .ops = &msm8952_quin_mi2s_be_ops,
	        .ignore_suspend = 1,
	},
	{
	        .name = "nau8540codec31",
	        .stream_name = "nau8540codec32",
	        .cpu_dai_name = "snd-soc-dummy-dai",
	        .codec_dai_name = "nau8540_2-hifi",
	        .codec_name = "nau8540_2.4-001c",
	        .no_pcm = 1,
	        .dpcm_capture = 1,
	        .be_id = MSM_BACKEND_DAI_NAU8543_2_TX,
	        .ops = &msm8952_quin_mi2s_be_ops,
	        .ignore_suspend = 1,
	},
...
parse_mclk_freq:
	...
	if (ret < 0)
		pr_err("%s:  doesn't support external speaker pa\n",
				__func__);
    ret = is_fpga_reset_gpio_support(pdev,pdata);
	if (ret < 0)
		pr_err("%s:  doesn't support fpga reset gpio\n",
				__func__);
