...
//liyongfeng add for ext-pa-en start
int msm_gpioset_ex_pa_en(enum pinctrl_client client,int enable)
{
	int ret;
	 if(enable)
	 	{
	 			ret =pinctrl_select_state(pinctrl_info[client].pinctrl,
			  pinctrl_info[CLIENT_WCD_INT].cdc_lines[17]);
	 	}
	 	else
	 	{
	 		 ret =pinctrl_select_state(pinctrl_info[client].pinctrl,
			 pinctrl_info[CLIENT_WCD_INT].cdc_lines[16]);	 	
	 	}
	return ret;
};
EXPORT_SYMBOL(msm_gpioset_ex_pa_en);
//liyongfeng add for ext-pa-en end

int msm_gpioset_ex_pa_vdd(enum pinctrl_client client,int enable)
{
	int ret;
	 if(enable)
	 	{
	 			ret =pinctrl_select_state(pinctrl_info[client].pinctrl,
			  pinctrl_info[CLIENT_WCD_INT].cdc_lines[20]);
	 	}
	 	else
	 	{
	 		 ret =pinctrl_select_state(pinctrl_info[client].pinctrl,
			 pinctrl_info[CLIENT_WCD_INT].cdc_lines[21]);	 	
	 	}
	return ret;
};
EXPORT_SYMBOL(msm_gpioset_ex_pa_vdd);
int msm_fpga_reset_en(enum pinctrl_client client,int enable)
{
	int ret;
	 if(enable)
		{
		  ret =pinctrl_select_state(pinctrl_info[client].pinctrl,
		  pinctrl_info[CLIENT_WCD_INT].cdc_lines[18]);
		}
		else
		{
		  ret =pinctrl_select_state(pinctrl_info[client].pinctrl,
		  pinctrl_info[CLIENT_WCD_INT].cdc_lines[19]);	 	
		}
	return ret;
};
EXPORT_SYMBOL(msm_fpga_reset_en);
