...
static const struct nau8540_fll_attr mclk_src_scaling_2[] = {
	{ 1, 0x0 },
	{ 2, 0x2 },
	{ 4, 0x3 },
	{ 8, 0x4 },
...
static const struct nau8540_fll_attr fll_ratio_2[] = {
	{ 512000, 0x01 },
	{ 256000, 0x02 },
	{ 128000, 0x04 },
	{ 64000, 0x08 },
...
static const struct nau8540_fll_attr fll_pre_scalar_2[] = {
	{ 1, 0x0 },
	{ 2, 0x1 },
	{ 4, 0x2 },
	{ 8, 0x3 },
...
/* over sampling rate */
static const struct nau8540_osr_attr osr_adc_sel_2[] = {
	{ 32, 3 },	/* OSR 32, SRC 1/8 */
	{ 64, 2 },	/* OSR 64, SRC 1/4 */
...
static const struct reg_default nau8540_2_reg_defaults[] = {
				...
				struct device_attribute *attr, char *buf)
{
	...
	for (index=0; index<ARRAY_SIZE(nau8540_2_reg_defaults); index++)
	{
		reg = nau8540_2_reg_defaults[index].reg;
		def_val = nau8540_2_reg_defaults[index].def;
...
static bool nau8540_2_readable_reg(struct device *dev, unsigned int reg)
{
	switch (reg) {
...
static bool nau8540_2_writeable_reg(struct device *dev, unsigned int reg)
{
	switch (reg) {
...
static bool nau8540_2_volatile_reg(struct device *dev, unsigned int reg)
{
	switch (reg) {
...
static const struct snd_kcontrol_new nau8540_2_snd_controls[] = {
	SOC_SINGLE_TLV("Mic9 Volume", NAU8540_REG_DIGITAL_GAIN_CH1,
		0, 0x520, 0, adc_vol_tlv),
	SOC_SINGLE_TLV("Mic10 Volume", NAU8540_REG_DIGITAL_GAIN_CH2,
		0, 0x520, 0, adc_vol_tlv),
	SOC_SINGLE_TLV("Mic11 Volume", NAU8540_REG_DIGITAL_GAIN_CH3,
		0, 0x520, 0, adc_vol_tlv),
	SOC_SINGLE_TLV("Mic12 Volume", NAU8540_REG_DIGITAL_GAIN_CH4,
		0, 0x520, 0, adc_vol_tlv),
	
	SOC_SINGLE("Mic9 Filter Enable",NAU8540_REG_HPF_FILTER_CH12,
		4, 1, 0),
	SOC_SINGLE("Mic10 Filter Enable",NAU8540_REG_HPF_FILTER_CH12,
		12, 1, 0),
	SOC_SINGLE("Mic11 Filter Enable",NAU8540_REG_HPF_FILTER_CH34,
		4, 1, 0),
	SOC_SINGLE("Mic12 Filter Enable",NAU8540_REG_HPF_FILTER_CH34,
		12, 1, 0),

	SOC_SINGLE_TLV("Mic9 Filter",NAU8540_REG_HPF_FILTER_CH12,
		0, 7, 0, hfp_filter),
	SOC_SINGLE_TLV("Mic10 Filter",NAU8540_REG_HPF_FILTER_CH12,
		8, 7, 0, hfp_filter),
	SOC_SINGLE_TLV("Mic11 Filter",NAU8540_REG_HPF_FILTER_CH34,
		0, 7, 0, hfp_filter),
	SOC_SINGLE_TLV("Mic12 Filter",NAU8540_REG_HPF_FILTER_CH34,
		8, 7, 0, hfp_filter),

	SOC_SINGLE_TLV("Frontend PGA9 Volume", NAU8540_REG_FEPGA3,
		0, 0x25, 0, fepga_gain_tlv),
	SOC_SINGLE_TLV("Frontend PGA10 Volume", NAU8540_REG_FEPGA3,
		8, 0x25, 0, fepga_gain_tlv),
	SOC_SINGLE_TLV("Frontend PGA11 Volume", NAU8540_REG_FEPGA4,
		0, 0x25, 0, fepga_gain_tlv),
	SOC_SINGLE_TLV("Frontend PGA12 Volume", NAU8540_REG_FEPGA4,
		8, 0x25, 0, fepga_gain_tlv),
};
...
static const struct snd_soc_dapm_widget nau8540_2_dapm_widgets[] = {
...
static const struct snd_soc_dapm_route nau8540_2_dapm_routes[] = {
...
static int nau8540_2_clock_check(struct nau8540 *nau8540, int rate, int osr)
{
	int osrate;

	if (osr >= ARRAY_SIZE(osr_adc_sel_2))
		return -EINVAL;
	osrate = osr_adc_sel_2[osr].osr;
...
static int nau8540_2_hw_params(struct snd_pcm_substream *substream,
	struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	if (nau8540_2_clock_check(nau8540, params_rate(params), osr))
		return -EINVAL;
	...
	regmap_update_bits(nau8540->regmap, NAU8540_REG_CLOCK_SRC,
		...
		osr_adc_sel_2[osr].clk_src << NAU8540_CLK_ADC_SRC_SFT);
...
static int nau8540_2_set_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	struct snd_soc_codec *codec = dai->codec;
	struct nau8540 *nau8540 = snd_soc_codec_get_drvdata(codec);
 ...
 */
static int nau8540_2_set_tdm_slot(struct snd_soc_dai *dai,
...
static const struct snd_soc_dai_ops nau8540_2_dai_ops = {
	.hw_params = nau8540_2_hw_params,
	.set_fmt = nau8540_2_set_fmt,
	.set_tdm_slot = nau8540_2_set_tdm_slot,
};
...
static struct snd_soc_dai_driver nau8540_2_dai = {
	.name = "nau8540_2-hifi",
	.capture = {
		.stream_name = "Capture",
		.channels_min = 1,
 ...
 */
static int nau8540_2_calc_fll_param(unsigned int fll_in,
	unsigned int fs, struct nau8540_fll *fll_param)
{
	 ...
	 */
	for (i = 0; i < ARRAY_SIZE(fll_pre_scalar_2); i++) {
		fref = fll_in / fll_pre_scalar_2[i].param;
		if (fref <= NAU_FREF_MAX)
			break;
	}
	if (i == ARRAY_SIZE(fll_pre_scalar_2))
		return -EINVAL;
	fll_param->clk_ref_div = fll_pre_scalar_2[i].val;

	/* Choose the FLL ratio based on FREF */
	for (i = 0; i < ARRAY_SIZE(fll_ratio_2); i++) {
		if (fref >= fll_ratio_2[i].param)
			break;
	}
	if (i == ARRAY_SIZE(fll_ratio_2))
		return -EINVAL;
	fll_param->ratio = fll_ratio_2[i].val;
	 ...
	 */
	fvco_max = 0;
	fvco_sel = ARRAY_SIZE(mclk_src_scaling_2);
	for (i = 0; i < ARRAY_SIZE(mclk_src_scaling_2); i++) {
		fvco = 256 * fs * 2 * mclk_src_scaling_2[i].param;
	...
	if (ARRAY_SIZE(mclk_src_scaling_2) == fvco_sel)
		return -EINVAL;
	fll_param->mclk_src = mclk_src_scaling_2[fvco_sel].val;
...
static void nau8540_2_fll_apply(struct regmap *regmap,
...
static int nau8540_2_set_pll(struct snd_soc_codec *codec, int pll_id, int source,
		unsigned int freq_in, unsigned int freq_out)
{
	...
	fs = freq_out / 256;
	ret = nau8540_2_calc_fll_param(freq_in, fs, &fll_param);
	if (ret < 0) {
	...
	nau8540_2_fll_apply(nau8540->regmap, &fll_param);
	mdelay(2);
	regmap_update_bits(nau8540->regmap, NAU8540_REG_CLOCK_SRC,
...
static int nau8540_2_config_sysclk(struct nau8540 *nau8540,
...
static int nau8540_2_set_sysclk(struct snd_soc_codec *codec,
	int clk_id, int source, unsigned int freq, int dir)
{
	struct nau8540 *nau8540 = snd_soc_codec_get_drvdata(codec);

	return nau8540_2_config_sysclk(nau8540, clk_id, freq);
}

static void nau8540_2_reset_chip(struct regmap *regmap)
{
	regmap_write(regmap, NAU8540_REG_SW_RESET, 0x00);
	regmap_write(regmap, NAU8540_REG_SW_RESET, 0x00);
}

static void nau8540_2_init_regs(struct nau8540 *nau8540)
{
	struct regmap *regmap = nau8540->regmap;
...
static struct snd_soc_codec_driver nau8540_2_codec_driver = {
	.set_sysclk = nau8540_2_set_sysclk,
	.set_pll = nau8540_2_set_pll,
	.suspend_bias_off = true,

	.component_driver = {
		.controls = nau8540_2_snd_controls,
		.num_controls = ARRAY_SIZE(nau8540_2_snd_controls),
		/*.dapm_widgets = nau8540_2_dapm_widgets,*/
		/*.num_dapm_widgets = ARRAY_SIZE(nau8540_2_dapm_widgets),*/
		/*.dapm_routes = nau8540_2_dapm_routes,*/
		/*.num_dapm_routes = ARRAY_SIZE(nau8540_2_dapm_routes),*/
	},
};

static const struct regmap_config nau8540_2_regmap_config = {
	.val_bits = 16,
	.reg_bits = 16,

	.max_register = NAU8540_REG_MAX,
	.readable_reg = nau8540_2_readable_reg,
	.writeable_reg = nau8540_2_writeable_reg,
	.volatile_reg = nau8540_2_volatile_reg,

	.cache_type = REGCACHE_RBTREE,
	.reg_defaults = nau8540_2_reg_defaults,
	.num_reg_defaults = ARRAY_SIZE(nau8540_2_reg_defaults),
};
int has_three_codec(void)
{
	return is_n8au540_2_exsit;
}

static int nau8540_2_i2c_probe(struct i2c_client *i2c,
	const struct i2c_device_id *id)
{
	...
	uint reg;
	uint def_val;
	pr_debug("nau8540_2 probe start \n");
	...
	nau8540->regmap = devm_regmap_init_i2c(i2c, &nau8540_2_regmap_config);
	if (IS_ERR(nau8540->regmap))
		return PTR_ERR(nau8540->regmap);
	nau8540->dev = dev;
	nau8540_2_reset_chip(nau8540->regmap);
	ret = regmap_read(nau8540->regmap, NAU8540_REG_I2C_DEVICE_ID, &value);

	if (ret < 0) {
		dev_err(dev, "Failed to read device id from the NAU85L40: %d\n",ret);
		is_n8au540_2_exsit=0;
		goto fail;
	}
	else{
	pr_debug("nau8540_2  device_id = %d \n",value);
	nau8540_2_init_regs(nau8540);
  for (index=0; index<ARRAY_SIZE(nau8540_2_reg_defaults); index++)
	{
		reg = nau8540_2_reg_defaults[index].reg;
		def_val = nau8540_2_reg_defaults[index].def;
	...
	for (attr_count = 0; attr_count < ARRAY_SIZE(attrs); attr_count++) {
  	...
  	pr_debug("enter NAU85L40-2 probe end \n");
	}
fail:	return snd_soc_register_codec(dev,
		&nau8540_2_codec_driver, &nau8540_2_dai, 1);
}

static int nau8540_2_i2c_remove(struct i2c_client *client)
{
...
static const struct i2c_device_id nau8540_2_i2c_ids[] = {
	{ "nau8540_2", 0 },
	{ }
};
MODULE_DEVICE_TABLE(i2c, nau8540_2_i2c_ids);

#ifdef CONFIG_OF
static const struct of_device_id nau8540_2_of_ids[] = {
	{ .compatible = "nuvoton,nau8540_2" },
	{}
};
MODULE_DEVICE_TABLE(of, nau8540_2_of_ids);
#endif

static struct i2c_driver nau8540_2_i2c_driver = {
	.driver = {
		.name = "nau8540_2",
		.of_match_table = of_match_ptr(nau8540_2_of_ids),
	},
	.probe = nau8540_2_i2c_probe,
	.remove = nau8540_2_i2c_remove,
	.id_table = nau8540_2_i2c_ids,
};
module_i2c_driver(nau8540_2_i2c_driver);
