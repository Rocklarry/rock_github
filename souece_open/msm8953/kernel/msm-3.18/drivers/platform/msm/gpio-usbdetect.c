...
static irqreturn_t gpio_usbdetect_vbus_irq(int irq, void *data)
{
	...
	if(usb->wakeup)
	{
		input_report_key(usb->input_dev, KEY_WAKEUP, 1);
		input_sync(usb->input_dev);
		input_report_key(usb->input_dev, KEY_WAKEUP, 0);
		input_sync(usb->input_dev);
	}
...
static int gpio_usbdetect_probe(struct platform_device *pdev)
{
	...
	usb->wakeup = of_property_read_bool(pdev->dev.of_node, "usb-wakeup");
	if(usb->wakeup)
	{
		usb->input_dev = input_allocate_device();
		if(!usb->input_dev)
		{
			usb->wakeup = 0;
		...
		{
			usb->input_dev->name = "gpio_usbdetect";
			usb->input_dev->phys = "input/usbdetect";
			usb->input_dev->id.bustype = BUS_USB;
			set_bit(EV_KEY, usb->input_dev->evbit);
			set_bit(EV_SYN, usb->input_dev->evbit);
			set_bit(KEY_WAKEUP, usb->input_dev->keybit);
			rc = input_register_device(usb->input_dev);
			if(rc)
			{
				input_free_device(usb->input_dev);
				usb->input_dev = NULL;
			}
		}
	}
...
static int gpio_usbdetect_remove(struct platform_device *pdev)
{
	...
	if(usb->input_dev)
	{
		input_unregister_device(usb->input_dev);
		input_free_device(usb->input_dev);
		usb->input_dev = NULL;
	}
