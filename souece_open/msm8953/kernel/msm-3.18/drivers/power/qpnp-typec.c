...
struct qpnp_typec_chip {
	...
	struct typec_wakeup_source	role_reversal_wakeup_source;
       ...
       bool			ft_test_mode;   //added by chenjing 20190507 for sc826-cn-01 ft test
};
				...
				enum cc_line_state cc_line)
{
	...
	if (cc_line != chip->cc_line_state) {
		...
		case OPEN:
			...
			} else if (gpio_is_valid(chip->ss_mux_supply_gpio)) {
				gpio_direction_output(chip->ss_mux_supply_gpio, 0);
			}
		...
		case CC_2:
			if (chip->ss_mux_vreg) {
				...
				gpio_direction_output(chip->ss_mux_supply_gpio, 1);
			}
...
static int qpnp_typec_parse_dt(struct qpnp_typec_chip *chip)
{
	...
	} else if (of_find_property(node, "qcom,ss-mux-supply-gpio", NULL)) {
		chip->ss_mux_supply_gpio = of_get_named_gpio(node, "qcom,ss-mux-supply-gpio", 0);
		if (!gpio_is_valid(chip->ss_mux_supply_gpio)) {
			pr_err("failed to get ss-mux power supply gpio.\n");
			return chip->ss_mux_supply_gpio;
		}

		rc = devm_gpio_request(chip->dev, chip->ss_mux_supply_gpio,
				"typec_ss_mux_supply_gpio");
		if (rc) {
			pr_err("failed to request ssmux power supply gpio: rc = %d\n", rc);
			chip->ss_mux_supply_gpio = -1;
			return rc;
		}
	}
	...
	chip->role_reversal_supported = of_property_read_bool(node,
       ...
       //added by chenjing 20190507 for sc826-cn-01 ft test
       chip->ft_test_mode = of_property_read_bool(node,
					"fibocom,ft-test-mode");

       pr_err("qpnp_typec_parse_dt: ft = %d\n", chip->ft_test_mode);
