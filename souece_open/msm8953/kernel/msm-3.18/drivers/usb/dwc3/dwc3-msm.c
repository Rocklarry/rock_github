...
struct dwc3_msm {
	...
	struct regulator	*hub_vddio;
	int 			vbus_en_gpio;
	int				vbus_en_gpio_bak;
	enum of_gpio_flags	vbus_en_lvl;
	int				hub_ext_id;
	int 			hub_5v_en_gpio;
	...
	struct delayed_work	usb_ext_id_dwork;
	struct delayed_work	hub_ext_id_dwork;
...
//added by chenjing 20190612 for typec-otg mantis 0023020
int dwc3_msm_typec_dfp_state(struct dwc3_msm *mdwc)
{
    if (mdwc->typec_psy)
    {
        union power_supply_propval val;
        int ret;
        ret = mdwc->typec_psy->get_property(mdwc->typec_psy,
            POWER_SUPPLY_PROP_TYPE, &val);

        dev_dbg(mdwc->dev, "typec_dfp_state = %d,%d\n", val.intval, ret);
...
static const char *chg_to_string(enum dwc3_chg_type chg_type)
{
	...
	case DWC3_FLOATED_CHARGER:	return "USB_FLOAT_CHARGER";
				  ...
				  const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_USB_OTG:
       {
              bool typec_event = false;   //added by chenjing 20190612 for typec-otg mantis 0023020
		id = val->intval ? DWC3_ID_GROUND : DWC3_ID_FLOAT;

              //added by chenjing 20190612 for typec-otg mantis 0023020
              if (mdwc->typec_otg_support)
              {
                    if (POWER_SUPPLY_TYPE_DFP == dwc3_msm_typec_dfp_state(mdwc))
                    {
        		     gpio_set_value(mdwc->hub_5v_en_gpio, !mdwc->vbus_en_lvl);	
                          gpio_set_value(mdwc->vbus_en_gpio_bak, !mdwc->vbus_en_lvl);
                          
                          mdwc->vbus_en_gpio = mdwc->vbus_en_gpio_bak;
                          typec_event = true;
                    }
              }
              
		if (!typec_event && mdwc->id_state == id)
			break;
	...
	case POWER_SUPPLY_PROP_PRESENT:
		...
		if (dwc->is_drd && !mdwc->in_restart) {
			...
			if(gpio_is_valid(mdwc->hub_ext_id))
			{
				queue_delayed_work(mdwc->dwc3_wq,
						&mdwc->resume_work, msecs_to_jiffies(2000));
			}
			else
			{
				queue_delayed_work(mdwc->dwc3_wq,
...
#define USB_ID_WORK_DELAY_TIME 2000   //2s
#define HUB_ID_WORK_DELAY_TIME 2000   //2s
static void dwc3_hub_id_work(struct work_struct *w)
{
	struct dwc3_msm *mdwc = container_of(w, struct dwc3_msm, hub_ext_id_dwork.work);
	struct power_supply *usb_psy = power_supply_get_by_name("usb");
	int hub_ext_id_level;

	if(usb_psy)
	{
		hub_ext_id_level = gpio_get_value(mdwc->hub_ext_id);
		mdwc->vbus_en_gpio = mdwc->hub_5v_en_gpio;
		power_supply_set_usb_otg(usb_psy, !hub_ext_id_level);
	}
}

static void dwc3_usb_id_work(struct work_struct *w)
{
	struct dwc3_msm *mdwc = container_of(w, struct dwc3_msm, usb_ext_id_dwork.work);
	struct power_supply *usb_psy = power_supply_get_by_name("usb");
	int usb_ext_id_level;
	int hub_ext_id_level;

	if(usb_psy)
	{
		//usb_ext_id_level = gpio_get_value(mdwc->usb_ext_id);
              usb_ext_id_level = !!irq_read_line(gpio_to_irq(mdwc->usb_ext_id)); 
		if(gpio_is_valid(mdwc->hub_ext_id))
			hub_ext_id_level = gpio_get_value(mdwc->hub_ext_id);

		pr_debug("%s:%d %d\n", __func__, usb_ext_id_level, hub_ext_id_level);

		mdwc->vbus_en_gpio = mdwc->vbus_en_gpio_bak;
		power_supply_set_usb_otg(usb_psy, !usb_ext_id_level);

		if(usb_ext_id_level && !hub_ext_id_level)
			schedule_delayed_work(&mdwc->hub_ext_id_dwork, msecs_to_jiffies(HUB_ID_WORK_DELAY_TIME));
	}
}

static irqreturn_t hub_ext_id_irq_handler(int irq, void *data)
{
	struct dwc3_msm *mdwc = data;
	int usb_ext_id_level;
	int hub_ext_id_level;
	struct power_supply *usb_psy = power_supply_get_by_name("usb");
#if 0
	int hub_id_work_delay_time = 0;
#endif

	if(usb_psy)
	{
		hub_ext_id_level = gpio_get_value(mdwc->hub_ext_id);
		if(gpio_is_valid(mdwc->usb_ext_id))
			//usb_ext_id_level = gpio_get_value(mdwc->usb_ext_id);
                     usb_ext_id_level = !!irq_read_line(gpio_to_irq(mdwc->usb_ext_id)); 
...
static irqreturn_t usb_ext_id_irq_handler(int irq, void *data)
{
	struct dwc3_msm *mdwc = data;
	int usb_ext_id_level;
	int hub_ext_id_level = 1;
	struct power_supply *usb_psy = power_supply_get_by_name("usb");
	int usb_id_work_delay_time = 0;;
	...
	if(usb_psy)
	{
		...
		if(!usb_ext_id_level)
		{
			if(!hub_ext_id_level) //hub is plugged in
			{
				//plug out hub devices
				mdwc->vbus_en_gpio = mdwc->hub_5v_en_gpio;
				power_supply_set_usb_otg(usb_psy, 0);
				usb_id_work_delay_time = USB_ID_WORK_DELAY_TIME;
			}
		}

		if(!usb_id_work_delay_time)
		{
			mdwc->vbus_en_gpio = mdwc->vbus_en_gpio_bak;
			power_supply_set_usb_otg(usb_psy, !usb_ext_id_level);
		}
		...
		{
			schedule_delayed_work(&mdwc->usb_ext_id_dwork,
					msecs_to_jiffies(usb_id_work_delay_time));
		}
	}
...
//added by chenjing 20190612 for typec-otg mantis 0023020
static void dwc3_msm_determine_initial_status(struct dwc3_msm *mdwc)
{
    if (POWER_SUPPLY_TYPE_DFP == dwc3_msm_typec_dfp_state(mdwc))
    {
        struct power_supply *usb_psy = power_supply_get_by_name("usb");
        if (usb_psy)
        {
            ...
            dbg_event(0xFF, "typec_id_state", 0);
        }
    }
}

static int dwc3_msm_debug_show(struct seq_file *s, void *unused)
{
    struct dwc3_msm *mdwc = s->private;
    int  hub_5v_gpio = gpio_get_value(mdwc->hub_5v_en_gpio);
    int  gpio_bak = gpio_get_value(mdwc->vbus_en_gpio_bak);
    
    seq_printf(s, "5v:%d,bak:%d\n", hub_5v_gpio, gpio_bak);
    return 0;
}

static int dwc3_msm_debug_open(struct inode *inode, struct file *file)
{
	return single_open(file, dwc3_msm_debug_show, inode->i_private);
}


static ssize_t dwc3_msm_debug_write(struct file *file, const char __user *ubuf,
...
const struct file_operations dwc3_msm_debug_fops = {
	.open = dwc3_msm_debug_open,
	.read = seq_read,
	.write = dwc3_msm_debug_write,
	.llseek = seq_lseek,
	.release = single_release,
};

static struct dentry * dwc3_msm_dbg_root;

static int dwc3_msm_debugfs_init(struct dwc3_msm *mdwc)
{
    struct dentry * dwc3_msm_dentry;
    dwc3_msm_dbg_root = debugfs_create_dir("dwc3_msm", NULL);

    if (!dwc3_msm_dbg_root || IS_ERR(dwc3_msm_dbg_root))
    	return -ENODEV;

    dwc3_msm_dentry = debugfs_create_file("debug", S_IRWXU|S_IRWXG|S_IRWXO,
				dwc3_msm_dbg_root, mdwc, &dwc3_msm_debug_fops);

    if (!dwc3_msm_dentry) 
    {
        debugfs_remove_recursive(dwc3_msm_dbg_root);
        return -ENODEV;
    }
...
static int dwc3_msm_probe(struct platform_device *pdev)
{
	...
	u32 val;
	char *usb_ext_id_str = "qcom,usb-ext-id";
	char *vbus_en_gpio_str = "qcom,vbus-en-gpio";
	struct pinctrl *pinctrl;
	struct pinctrl_state *usb_ext_id_state;
	...
	INIT_DELAYED_WORK(&mdwc->usb_ext_id_dwork, dwc3_usb_id_work);
	INIT_DELAYED_WORK(&mdwc->hub_ext_id_dwork, dwc3_hub_id_work);
	...
	mdwc->charging_disabled = of_property_read_bool(node,
      ...
      //added by chenjing 20190612 for typec-otg mantis 0023020
      mdwc->typec_otg_support = of_property_read_bool(node,
				"ght,typec-otg-enable");

      dev_err(&pdev->dev, "typec-otg:%d,%d\n", mdwc->typec_otg_support, mdwc->charging_disabled);

      if (mdwc->typec_otg_support)
      {
            mdwc->typec_psy = power_supply_get_by_name("typec");
      }
	...
	if(!IS_ERR(pinctrl))
	{
		usb_ext_id_state = pinctrl_lookup_state(pinctrl, "default");
		pinctrl_select_state(pinctrl, usb_ext_id_state);
	}


	mdwc->vbus_en_gpio = -1;
	mdwc->vbus_en_gpio_bak = -1;
	mdwc->usb_ext_id = -1;

	if((mdwc->hub_5v_en_gpio = of_get_named_gpio(node, "qcom,hub-5v-en-gpio", 0)) > 0)
	{
		ret = gpio_request(mdwc->hub_5v_en_gpio, "hub_5v_en");
		if(ret < 0)
		{
			mdwc->hub_5v_en_gpio = -1;
			dev_err(&pdev->dev, "request vbus en gpio %d failed.\n",
					mdwc->hub_5v_en_gpio);
		}
	}

	mdwc->hub_vddio = devm_regulator_get(mdwc->dev, "hub-vddio");
	if(!IS_ERR_OR_NULL(mdwc->hub_vddio))
	{
		ret = regulator_enable(mdwc->hub_vddio);
		if(ret)
		{
			dev_err(mdwc->dev, "unable to enable hub vddio");
		}
	}

	if((mdwc->hub_ext_id = of_get_named_gpio(node, "qcom,hub-ext-id", 0)) > 0)
	{
		ret = gpio_request(mdwc->hub_ext_id, "hub_ext_id");
		if(ret < 0)
		{
			mdwc->hub_ext_id = -1;
			dev_err(&pdev->dev, "request vbus en gpio %d failed.\n",
		...
		{
			ret = request_irq( gpio_to_irq(mdwc->hub_ext_id),
					hub_ext_id_irq_handler,
					IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING,
					"hub_ext_id", mdwc); 
		}
	}

	if(of_property_read_bool(node, usb_ext_id_str))
	{
		mdwc->usb_ext_id = of_get_named_gpio(node, usb_ext_id_str, 0);
		if(gpio_is_valid(mdwc->usb_ext_id))
		{
			if(of_property_read_bool(node, vbus_en_gpio_str))
			{
				mdwc->vbus_en_gpio_bak = of_get_named_gpio_flags(
						node, vbus_en_gpio_str, 0, &mdwc->vbus_en_lvl);
				ret = gpio_request(mdwc->vbus_en_gpio_bak, "vbus en gpio");
				if(ret < 0)
				{
					mdwc->vbus_en_gpio_bak = -1;
					dev_err(&pdev->dev, "request vbus en gpio %d failed.\n",
							mdwc->vbus_en_gpio_bak);
				}
			}

			ret = gpio_request(mdwc->usb_ext_id, "usb ext id");
			if(!ret)
			{
				ret = request_irq( gpio_to_irq(mdwc->usb_ext_id),
						usb_ext_id_irq_handler,
						IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING,
						"usb_ext_id", mdwc); 

				usb_ext_id_irq_handler(gpio_to_irq(mdwc->usb_ext_id), mdwc);
			}
			else
			{
				mdwc->usb_ext_id = -1;
				dev_err(&pdev->dev, "request usb ext id gpio %d failed.\n", mdwc->usb_ext_id);
			}
		}
	}

	if(gpio_is_valid(mdwc->hub_ext_id))
		hub_ext_id_irq_handler(gpio_to_irq(mdwc->hub_ext_id), mdwc);
	...
	if (!dwc->is_drd && host_mode) {
       ...
       (void)dwc3_msm_debugfs_init(mdwc);

       //added by chenjing 20190612 for typec-otg mantis 0023020
       dwc3_msm_determine_initial_status(mdwc);
...
static int dwc3_msm_remove(struct platform_device *pdev)
{
	...
	else if (gpio_is_valid(mdwc->vbus_en_gpio))
		gpio_set_value(mdwc->vbus_en_gpio, !mdwc->vbus_en_lvl);	
...
static int dwc3_otg_start_host(struct dwc3_msm *mdwc, int on)
{
	...
	if (!mdwc->vbus_reg) {
		...
		if (IS_ERR(mdwc->vbus_reg) &&
				...
				&& !gpio_is_valid(mdwc->vbus_en_gpio)) {
	...
	if (on) {
		...
		if (!IS_ERR(mdwc->vbus_reg))
			ret = regulator_disable(mdwc->vbus_reg);
                ...
                {      
                    if (mdwc->typec_otg_support)   //added by chenjing 20190612 for typec-otg mantis 0023020
                    {
                        gpio_set_value(mdwc->hub_5v_en_gpio, !mdwc->vbus_en_lvl);	
                        gpio_set_value(mdwc->vbus_en_gpio_bak, !mdwc->vbus_en_lvl);	
                        dev_err(mdwc->dev, "vbus_en_gpio 4:%d, !mdwc->vbus_en_lvl\n", mdwc->vbus_en_gpio);
                    }
                    else
                    {
			    gpio_set_value(mdwc->vbus_en_gpio, !mdwc->vbus_en_lvl);	
...
static void dwc3_otg_sm_work(struct work_struct *w)
{
	...
	case OTG_STATE_B_IDLE:
		if (!test_bit(ID, &mdwc->inputs)) {
			...
			/* fall through */
			default:
                   //begin added by chenjing 20190507:sc826-cn-01 not support charging, let the port into b_peripheral state,for FT USB_ID test
			       if (mdwc->charging_disabled)
                            {
                                pm_runtime_get_sync(mdwc->dev);
      				    dbg_event(0xFF, "NOCHG sync", atomic_read(&mdwc->dev->power.usage_count));
        			    dwc3_otg_start_peripheral(mdwc, 1);
        			    mdwc->otg_state = OTG_STATE_B_PERIPHERAL;
        			    work = 1;
                            }
                   //end added by chenjing 20190507:sc826-cn-01 not support charging, let the port into b_peripheral state,for FT USB_ID test
				break;
			}
		} else {
