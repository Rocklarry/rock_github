...
{	static unsigned int cnt=1;
	...
	pwm_bl_ght = ctrl->pwm_bl;
	}
	}else{
	ctrl->pwm_bl = pwm_bl_ght;
	}
	cnt++;
	ctrl->pwm_enabled = 0;
}
...
void audio_cntl(int ishigh) //wj
{
	/*
	int rc = 0;
	if (gpio_is_valid(91) ){
		rc = gpio_request(91,		"audio_enable gpio");
			if (rc) {
			pr_err("request audio gpio failed, rc=%d\n",
				       rc);
				       	if (gpio_is_valid(91))
		            gpio_free(91);
		    	rc = gpio_request(91,		"audio_enable req");
		    if(rc){
					goto out;
				}
			}
...
int mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	...
	if (enable) {
		...
		if (gpio_is_valid(ctrl_pdata->bklt_en_gpio)) {
			...
			gpio_free(ctrl_pdata->bklt_en_gpio);
			pr_err("%s: =====%d disable to set dir for bklt gpio\n",__func__,__LINE__);
		}
							...
							u32 bl_level)
{
	...
	struct mdss_dsi_ctrl_pdata *sctrl = NULL;
         bl_level +=160; 
                if(bl_level>3860) bl_level=3860;///wjadd
...
static int mdss_dsi_panel_on(struct mdss_panel_data *pdata)
{
	...
	pr_err("stephen to mdss_dsi_panel_on\n ");
	if (ctrl->cmd_access == CMD_ACCESS_DSI) {
		pr_err("stephen mdss_dsi_panel_on to CMD_ACCESS_DSI\n");
		if (ctrl->on_cmds.cmd_cnt){
			pr_err("stephen mdss_dsi_panel_on to CMD_ACCESS_DSI to cmd\n");
			 mdss_mipi_i2c_init(ctrl,0);
			 mdss_dsi_panel_cmds_send(ctrl, &ctrl->on_cmds, CMD_REQ_COMMIT);
			}
	} else if (ctrl->cmd_access == CMD_ACCESS_I2C) {
		//add by eliot shao 
		pr_err("stephen mdss_dsi_panel_on to CMD_ACCESS_I2C to i2c init ==\n");
		mdss_mipi_i2c_init(ctrl,0);
		pr_err("stephen mdss_mipi_i2c_init to mdss_dsi_panel_cmds_send\n");
		 mdss_dsi_panel_cmds_send(ctrl, &ctrl->on_cmds, CMD_REQ_COMMIT);
