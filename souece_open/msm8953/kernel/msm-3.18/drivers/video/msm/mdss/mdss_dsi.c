...
int mdss_dsi_on(struct mdss_panel_data *pdata)
{
	...
	ret = mdss_dsi_panel_power_ctrl(pdata, MDSS_PANEL_POWER_ON);//上电
	if (ret) {
...
static int mdss_dsi_unblank(struct mdss_panel_data *pdata)
{
	...
	if (!(ctrl_pdata->ctrl_state & CTRL_STATE_PANEL_INIT)) {
		if (!pdata->panel_info.dynamic_switch_pending) {
			...
			pr_err("%s: ======%d to initialize the panel\n",__func__,__LINE__);
			if (ret) {
				  ...
				  int event, void *arg)
{
	...
	case MDSS_EVENT_CHECK_PARAMS:
		...
		break;
	case MDSS_EVENT_LINK_READY: //上电操作
		if (ctrl_pdata->refresh_clk_rate)
			rc = mdss_dsi_clk_refresh(pdata,
				ctrl_pdata->update_phy_timing);

		rc = mdss_dsi_on(pdata);//上电 背光 lcd使能
		mdss_dsi_op_mode_config(pdata->panel_info.mipi.mode,
							pdata);
		pr_err("%s+:=====%d   MDSS_EVENT_LINK_READY ctrl=%d event=%d\n", __func__,__LINE__, ctrl_pdata->ndx, event);
		break;
	case MDSS_EVENT_UNBLANK:
		if (ctrl_pdata->on_cmds.link_state == DSI_LP_MODE)
			rc = mdss_dsi_unblank(pdata); //使能icn6211
		pr_err("%s+:=====%d   MDSS_EVENT_UNBLANK ctrl=%d event=%d\n", __func__,__LINE__, ctrl_pdata->ndx, event);
		break;
	case MDSS_EVENT_POST_PANEL_ON: //最后打开 panel on
		rc = mdss_dsi_post_panel_on(pdata);
		pr_err("%s+:=====%d   MDSS_EVENT_POST_PANEL_ON ctrl=%d event=%d\n", __func__,__LINE__, ctrl_pdata->ndx, event);
		
			
		//mdelay(5000);
		if (gpio_is_valid(ctrl_pdata->bklt_en_gpio)) {
			gpio_set_value((ctrl_pdata->bklt_en_gpio), 1);
			pr_err("%s+:======bklt_en_gpio =======%d\n", __func__,__LINE__);
		}
	...
	case MDSS_EVENT_PANEL_ON: 
		...
		pr_err("%s+:===== %d   MDSS_EVENT_PANEL_ON ctrl=%d event=%d\n", __func__,__LINE__, ctrl_pdata->ndx, event);
		...
		struct platform_device *pdev, char *panel_cfg)
{
	...
	struct mdss_panel_info *pinfo = &ctrl_pdata->panel_data.panel_info;
pr_err("stephen go to mdss_dsi_find_panel_of_node panel_cfg =%s\n",panel_cfg);
