...
//liyongfeng add  backlight  transform for power save start 20180605
static bool enable_light_transform=0;
static int light_transform[256] = {
0,1,1,1,2,2,3,3,4,4,
...
173,175,176,177,178,179,181,182,183,184,
185,187,188,189,190,191,193,194,195,196,
197,199,200,201,202,203,205,206,207,208,
209,211,212,213,214,215,217,218,219,220,
221,223,224,225,226,227,229,230,231,232,
233,235,236,237,238,240
};
//liyongfeng add  backlight  transform for power save end 20180605
				      ...
				      enum led_brightness value)
{
	...
	//liyongfeng add  backlight  transform for power save start 20180605
	if((enable_light_transform==1)&&(value>=0)&&(value<=255))
  {
  	  value=light_transform[value];
  	  pr_err("%s: value transforme=%d \n", __func__,value);
  }
  //liyongfeng add  backlight  transform for power save end 20180605
		...
		struct device_attribute *attr, char *buf)
{
	...
	ret = scnprintf(buf, PAGE_SIZE,
			...
			pinfo->panel_name, pinfo->is_prim_panel?1:1,
	...
	struct device_attribute *attr, const char *buf, size_t len)
{
	...
	if (1 || dfps_mode >= DFPS_MODE_MAX) {///wj
...
void audio_cntl(int ishigh);
void headset_report(bool in); ///WJ
static ssize_t set_audio_mode(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	int thermal_level = 0,rc;
	rc = kstrtoint(buf, 1, &thermal_level);
	 pr_err("=====wj %s ====set:%d\n",__FUNCTION__,thermal_level);
	  audio_cntl(thermal_level);
	  	return rc;
}
static ssize_t headsettst_store(struct device *dev,
        struct device_attribute *attr, const char *buf, size_t count)
{       int headsettst=1;
        sscanf(buf, "%du", &headsettst);
        headset_report(headsettst==4?1:0);
	pr_err("=====wj %s ====set:%d\n",__FUNCTION__,headsettst);
        return count;
}
...
static DEVICE_ATTR(msm_fb_dfps_mode, S_IRUGO | S_IWUSR,
	...
	static DEVICE_ATTR(audio_ght, S_IRUGO | S_IWUSR,
	 NULL, set_audio_mode);
static DEVICE_ATTR(headset, S_IRUGO | S_IWUSR,
         NULL, headsettst_store);
...
//liyongfeng add  backlight  transform for power save start 20180605
	enable_light_transform=of_property_read_bool(pdev->dev.of_node,
		"backlight-transform");
//liyongfeng add  backlight  transform for power save end 20180605
	...
	if(of_property_read_bool(pdev->dev.of_node, "qcom,pmi8952_enable"))
	{
		rc = of_property_read_string(pdev->dev.of_node, "qcom,pmi8952_enable",&backlight_led.name);
		if (!rc) {
			pr_debug("Use pmi8952 and set default backlight name to =%s\n",backlight_led.name);
		}
	}
	...
	mfd->panel_info->dynamic_fps = 0;///wj
	 audio_cntl(0);mdelay(10);
                audio_cntl(1);
