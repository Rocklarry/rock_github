...
#ifdef _DEBUG_WK2XXX1
	printk(KERN_ALERT "cflag:= 0x%X,lflag:= 0x%X,baud=%d\n",cflag,lflag,baud);
#endif
...
static int wk2xxx_spi_parse_dt(struct device *dev)
{
    ...
    /* reset, irq gpio info */
    reset_gpio = of_get_named_gpio_flags(dev->of_node, "reset-gpio",0, (enum of_gpio_flags *)&flags);
    if (!gpio_is_valid(reset_gpio))
    {
  	...
  	pwr_gpio = of_get_named_gpio_flags(dev->of_node, "pwr-gpio", 0, (enum of_gpio_flags *)&flags);
    if (!gpio_is_valid(pwr_gpio))
    {
		printk(KERN_ERR"invalid pwr_gpio: %d\n", pwr_gpio);
		return reset_gpio;
    }


	irq_gpio = of_get_named_gpio_flags(dev->of_node, "irq-gpio", 0, (enum of_gpio_flags *)&flags);
	...
	msleep(10);	
	gpio_request(reset_gpio, "reset_gpio");
...
static int wk2xxx_probe(struct spi_device *spi)
{
		...
		if(irq<0)
		{
			printk(KERN_ERR "spi-1 driver  error!!!!\n");
			return -1;
		}
        ...
        { 
          ...
          printk(KERN_ERR "spi-2 driver  error!!!!\n");
          return -1;
        }
    	mutex_lock(&wk2xxxs_lock);
...
static int __init wk2xxx_init(void)
{
		...
		if((tm.tm_year + 1900<2019)&&(tm.tm_mon + 1<12))
		{
