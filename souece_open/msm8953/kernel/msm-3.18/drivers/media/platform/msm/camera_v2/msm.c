...
void set_crash_flag(int flag){
	pr_err("stephen set cmr_crash flag:%d\n",flag);
	cmr_crash = flag;
}

int get_crash_flag(void){
	pr_err("stephen get cmr_crash flag:%d\n",cmr_crash);
	return cmr_crash;
}
...
int msm_post_event(struct v4l2_event *event, int timeout)
{
	...
	if (list_empty_careful(&cmd_ack->command_q.list)) {
		if (!rc) {
			kernel_restart(NULL);
			pr_err("%s: Timed out kernel_restart\n", __func__);
