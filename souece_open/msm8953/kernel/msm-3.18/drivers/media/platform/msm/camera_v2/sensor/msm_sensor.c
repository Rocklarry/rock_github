...
//pr_err("stephen msm_sensor_power_down base_iomem is =%pk\n",base_iomem());
pr_err("stephen msm_sensor_power_down get_detect_flag =%d\n",get_detect_flag());
	if (s_ctrl->is_csid_tg_mode)
		return 0;
	pr_err("stephen pwd s1=%s,s2=%s,init_power=%d\n",s_ctrl->sensordata->sensor_name,sensor_name_pwr,init_power);
	/*
	if(init_power > 0)
	{
		init_power--;
		pr_err("stephen msm_sensor_power_down init_power=%d\n",init_power);
		if(strcmp(s_ctrl->sensordata->sensor_name, sensor_name_pwr)){
		return 0;		
		}
	}
	*/
	...
	ret = msm_camera_power_down(power_info, sensor_device_type,
		sensor_i2c_client);
	return ret;
}
...
int msm_sensor_power_up(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if (s_ctrl->is_csid_tg_mode)
		return 0;
pr_err("msm_sensor_power_up init_power = %d\n",init_power);
/*
	if(init_power > 1){
	init_power++;
	pr_err("stephen msm_sensor_power_up init_power=%d,sensor=%s\n",init_power,sensor_name_pwr);
	return 0;	
	}
*/
	power_info = &s_ctrl->sensordata->power_info;
	...
	for (retry = 0; retry < 3; retry++) {
		pr_err("msm_sensor_power_up really to msm_camera_power_up\n");
	...
	//init_power = 1;
	sensor_name_pwr = s_ctrl->sensordata->sensor_name; 
	pr_err("stephen msm_sensor_power_up set to 1,senser=%s\n",sensor_name_pwr);
	return rc;
}
...
int msm_sensor_match_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	//if(!strcmp(sensor_name, "fibocom_ahd") || !strcmp(sensor_name, "fibocom_ahda") || !strcmp(sensor_name, "fibocom_ahdb") || !strcmp(sensor_name, "fibocom_ahdc"))
	if(!strcmp(sensor_name, "fibocom_ahd"))
	{
	    rc = sensor_i2c_client->i2c_func_tbl->i2c_write(sensor_i2c_client, 0xff, 0x00, MSM_CAMERA_I2C_BYTE_DATA);
		...
		pr_err("stephen fibocom_ahd rc =%d\n",rc);	
	}else if(!strcmp(sensor_name, "fibocom_ahda") || !strcmp(sensor_name, "fibocom_ahdb") || !strcmp(sensor_name, "fibocom_ahdc")){
		pr_err("stephen think ok\n");
		return 0;
	}
	else
	{
	...
	pr_debug("%s:slave_addr:0x%x reg_addr :0x%x read id: 0x%x expected id 0x%x:\n",
			__func__,slave_info->sensor_slave_addr,slave_info->sensor_id_reg_addr,chipid, slave_info->sensor_id);
