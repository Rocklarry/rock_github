...
static int usecount_csid0_cid_share = 0;
//static int usecount_csid0_cid4_share = 0;
static int csid0_cid0_cfg_flag = 0;
static int csid0_cid4_cfg_flag = 0;
static int csid0_cid8_cfg_flag = 0;
static int csid0_cid12_cfg_flag = 0;
	...
	struct csid_device *csid_dev)
{
	...
	for (i = 0; i < csid_lut_params->num_cid; i++) {
		...
		val = msm_camera_io_r(csid_dev->base +
			...
			8));
		pr_err("stephen msm_csid_cid_lut val=0x%x",val);
		val |= (csid_lut_params->vc_cfg[i]->dt <<
			((csid_lut_params->vc_cfg[i]->cid % 4) * 8));
		msm_camera_io_w(val, csid_dev->base +
			csid_dev->ctrl_reg->csid_reg.csid_cid_lut_vc_0_addr +
			(csid_lut_params->vc_cfg[i]->cid >> 2) * 4);
		pr_err("stephen msm_csid_cid_lut val1=0x%x",val);
		val = (csid_lut_params->vc_cfg[i]->decode_format << 4) | 0x3;
		msm_camera_io_w(val, csid_dev->base +
			csid_dev->ctrl_reg->csid_reg.csid_cid_n_cfg_addr +
			(csid_lut_params->vc_cfg[i]->cid * 4));
	}
	if(csid_lut_params->vc_cfg[0]->cid  == 0){
		
		cid_share_csid = 4;
		val = msm_camera_io_r(csid_dev->base +
			csid_dev->ctrl_reg->csid_reg.csid_cid_lut_vc_0_addr +
			(cid_share_csid >> 2) * 4)
			& ~(0xFF << ((cid_share_csid % 4) *
			8));
		val |= (csid_lut_params->vc_cfg[0]->dt <<
			((cid_share_csid % 4) * 8));
		msm_camera_io_w(val, csid_dev->base +
			csid_dev->ctrl_reg->csid_reg.csid_cid_lut_vc_0_addr +
			(cid_share_csid >> 2) * 4);

		val = (csid_lut_params->vc_cfg[0]->decode_format << 4) | 0x3;
		pr_err("stephen msm_csid_cid_lut val2=0x%x\n",val);
		msm_camera_io_w(val, csid_dev->base +
			csid_dev->ctrl_reg->csid_reg.csid_cid_n_cfg_addr +
			(cid_share_csid * 4));
//====
...
//===
	...
	if(csid_lut_params->vc_cfg[0]->cid  == 4){
		...
		val = (csid_lut_params->vc_cfg[0]->decode_format << 4) | 0x3;
		pr_err("stephen msm_csid_cid_lut val3=0x%x\n",val);
		msm_camera_io_w(val, csid_dev->base +
			csid_dev->ctrl_reg->csid_reg.csid_cid_n_cfg_addr +
			(cid_share_csid * 4));
//====
...
//===
	...
	if(csid_lut_params->vc_cfg[0]->cid  == 8){
...
//===
	...
	if(csid_lut_params->vc_cfg[0]->cid  == 12){
...
pr_err("stephen in msm_csid_reset\n");
...
pr_err("stephen in msm_csid_irq CAPTURE\n");
	if (!csid_dev) {
	...
	pr_err("stephen irq=0x%x,csid_rst_done_irq_bitshift=0x%x\n",irq,csid_dev->ctrl_reg->csid_reg.csid_rst_done_irq_bitshift);
	if (irq & (0x1 <<
		csid_dev->ctrl_reg->csid_reg.csid_rst_done_irq_bitshift)){
		pr_err("stephen csid reset complete\n");	
...
{
	uint32_t irq;
pr_err("stephen in msm_csid_release csid0_cid0_cfg_flag=%d,csid0_cid4_cfg_flag=0x%d,csid_dev->pdev->name=%s\n",csid0_cid0_cfg_flag,csid0_cid4_cfg_flag,csid_dev->pdev->name);
pr_err("stephen msm_csid_release csid_dev->pdev->id=%d\n",csid_dev->pdev->id);
//if(csid_dev->current_csid_params.lut_params.vc_cfg[0] != NULL){
//if(csid_dev->current_csid_params.lut_params.vc_cfg[0] != NULL){
//csid_dev->current_csid_params.lut_params.vc_cfg[0]->cid

/*
	if(csid0_cid0_cfg_flag == 1){
	pr_err("csid0_cid0_cfg_flag cid=%d\n",csid_dev->current_csid_params.lut_params.vc_cfg[0]->cid);
		if(csid_dev->current_csid_params.lut_params.vc_cfg[0]->cid == 0){
		pr_err("stephen relese csid0 cid0\n");
			if(usecount_csid0_cid_share > 0)	
			usecount_csid0_cid_share--;
		}
	}
	if(csid0_cid4_cfg_flag == 1){
	pr_err("csid0_cid4_cfg_flag cid=%d\n",csid_dev->current_csid_params.lut_params.vc_cfg[0]->cid);
		if(csid_dev->current_csid_params.lut_params.vc_cfg[0]->cid == 4){
		pr_err("stephen relese csid0 cid4\n");
			if(usecount_csid0_cid_share > 0)	
			usecount_csid0_cid_share--;
		}
	}
...
/*
	if((csid_dev->current_csid_params.lut_params.vc_cfg[0]->cid == 0) ||
		(csid_dev->current_csid_params.lut_params.vc_cfg[0]->cid == 4)){
		if((usecount_csid0_cid0_share != 0) || 
			(usecount_csid0_cid4_share != 0)){
			pr_err("stephen just return\n");
			return 0;
		}
	}
...
//}
	...
	if(csid_dev->pdev->id == 0){
		pr_err("stephen in csid0\n");
		if((csid0_cid4_cfg_flag == 1) || (csid0_cid0_cfg_flag == 1)
			||(csid0_cid8_cfg_flag == 1) || (csid0_cid12_cfg_flag == 1))
		{
			pr_err("stephen  decrease count\n");
			if(usecount_csid0_cid_share > 0)	
			usecount_csid0_cid_share--;
		}
		pr_err("stephen usecount_csid0_cid_share=%d\n",usecount_csid0_cid_share);
		if(usecount_csid0_cid_share != 0){
				pr_err("stephen just return\n");
			return 0;
			}
	...
	if (csid_dev->csid_state != CSID_POWER_UP) {
		pr_err("%s: csid invalid state %d\n", __func__,
			csid_dev->csid_state);
		pr_err("stephen msm_csid_release is down\n");
		return 0;
		//return -EINVAL;
	}
...
static int32_t msm_csid_cmd(struct csid_device *csid_dev, void __user *arg)
{
	...
	case CSID_CFG: {
		...
		//rc = msm_csid_config(csid_dev, &csid_params);//modidy stephen
		//add stephen ++
		if(csid_dev->current_csid_params.lut_params.vc_cfg[0]->cid == 0 ){
			csid0_cid0_cfg_flag = 1;
			usecount_csid0_cid_share++;
			pr_err("stephen cid0_share=%d\n",usecount_csid0_cid_share);
		}
		
		if(csid_dev->current_csid_params.lut_params.vc_cfg[0]->cid == 4 ){
			csid0_cid4_cfg_flag = 1;
			usecount_csid0_cid_share++;
			pr_err("stephen cid4_share=%d\n",usecount_csid0_cid_share);
		}
		if(csid_dev->current_csid_params.lut_params.vc_cfg[0]->cid == 8 ){
			csid0_cid8_cfg_flag = 1;
			usecount_csid0_cid_share++;
			pr_err("stephen cid8_share=%d\n",usecount_csid0_cid_share);
		}
		if(csid_dev->current_csid_params.lut_params.vc_cfg[0]->cid == 12 ){
			csid0_cid12_cfg_flag = 1;
			usecount_csid0_cid_share++;
			pr_err("stephen cid12_share=%d\n",usecount_csid0_cid_share);
		}
		...
		if((csid_dev->current_csid_params.lut_params.vc_cfg[0]->cid == 0)
			|| (csid_dev->current_csid_params.lut_params.vc_cfg[0]->cid == 4)
			|| (csid_dev->current_csid_params.lut_params.vc_cfg[0]->cid == 8)
			|| (csid_dev->current_csid_params.lut_params.vc_cfg[0]->cid == 12)){
				if( usecount_csid0_cid_share == 1){
					rc = msm_csid_config(csid_dev, &csid_params);
					pr_err("stephen first cid0-12 to msm_csid_config\n");
				}else{
					pr_err("stephen not  first cid0-12 no need\n");
				}
			}else{
				rc = msm_csid_config(csid_dev, &csid_params);
			}
...
//add stephen ++
/*
		msm_csid_init(csid_dev, &cdata->cfg.csid_version);
...
static int32_t msm_csid_cmd32(struct csid_device *csid_dev, void __user *arg)
{
	...
	case CSID_CFG: {
		...
		if(csid_dev->current_csid_params.lut_params.vc_cfg[0]->cid == 0 ){
			csid0_cid0_cfg_flag = 1;
			usecount_csid0_cid_share++;
			pr_err("stephen COMPAT cid0_share=%d\n",usecount_csid0_cid_share);
		}
		
		if(csid_dev->current_csid_params.lut_params.vc_cfg[0]->cid == 4 ){
			csid0_cid4_cfg_flag = 1;
			usecount_csid0_cid_share++;
			pr_err("stephen COMPAT cid4_share=%d\n",usecount_csid0_cid_share);
		}
		if(csid_dev->current_csid_params.lut_params.vc_cfg[0]->cid == 8 ){
			csid0_cid8_cfg_flag = 1;
			usecount_csid0_cid_share++;
			pr_err("stephen COMPAT cid8_share=%d\n",usecount_csid0_cid_share);
		}
		if(csid_dev->current_csid_params.lut_params.vc_cfg[0]->cid == 12 ){
			csid0_cid12_cfg_flag = 1;
			usecount_csid0_cid_share++;
			pr_err("stephen COMPAT cid12_share=%d\n",usecount_csid0_cid_share);
		}
