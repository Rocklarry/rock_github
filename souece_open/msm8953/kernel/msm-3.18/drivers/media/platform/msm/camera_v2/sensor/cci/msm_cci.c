...
//#define CCI_TIMEOUT msecs_to_jiffies(500)//modify stephen
//#define CCI_TIMEOUT msecs_to_jiffies(500*4)
...
pr_err("stephen msm_cci_process_full_q rc=%d\n",rc);
		//rc = 0;//the first camera must ok
		if (rc < 0) {
			pr_err("%s: %d failed rc %d\n", __func__, __LINE__, rc);
			//rc = msm_cci_wait(cci_dev, master, queue);//add stephen 
			//if(rc < 0)
			return rc;
		}
	} else {
	...
	struct msm_camera_cci_ctrl *c_ctrl)
{
	...
	cci_dev = v4l2_get_subdevdata(sd);
pr_err("stephen in msm_cci_init\n");
	if (!cci_dev || !c_ctrl) {
		pr_err("%s:%d failed: invalid params %pK %pK\n", __func__,
			__LINE__, cci_dev, c_ctrl);
		rc = -EINVAL;
...
static int32_t msm_cci_release(struct v4l2_subdev *sd)
{
	...
	if (!cci_dev->ref_count || cci_dev->cci_state != CCI_STATE_ENABLED) {
		...
		pr_err("stephen in msm_cci_release has down ahb_vote_suspend\n");
		return 0;
		//rc = -EINVAL;
	...
	unsigned int cmd, void *arg)
{
	...
	case MSM_SD_SHUTDOWN: {
		...
		pr_err("stephen start to MSM_CCI_RELEASE rc=%d\n",rc);
		
		ctrl_cmd.cmd = MSM_CCI_INIT; //modify stephen
		rc = msm_cci_config(sd, &ctrl_cmd);
		pr_err("stephen start to MSM_CCI_INIT rc=%d\n",rc);
