...
static void camera_pwr_set(int on){
	void __iomem *base;
	base = base_iomem();
	if(on == 0){
		writel(0x201, base + 0x1c000);  //gpio28--->0x1c
		writel(0x00,  base + 0x1c004);  //gpio28--->0x1c
		writel(0x201, base + 0x87000);  //gpio135--->0x87
		writel(0x00,  base + 0x87004);  //gpio135--->0x87
		writel(0x201, base + 0x8a000);  //gpio138--->0x8a
		writel(0x00,  base + 0x8a004);  //gpio138--->0x8a
		pr_err("cmaera pwr off\n");
	}else if(on == 1){
		writel(0x201,  base + 0x1c000);  //gpio28--->0x1c
		writel(0x03,   base + 0x1c004);  //gpio28--->0x1c
		writel(0x201,  base + 0x87000);  //gpio135--->0x87
		writel(0x03,   base + 0x87004);  //gpio135--->0x87
		writel(0x201,  base + 0x8a000);  //gpio138--->0x8a
		writel(0x03,   base + 0x8a004);  //gpio138--->0x8a
		pr_err("cmaera pwr on\n");
		}
}
	...
	struct msm_camera_i2c_client *sensor_i2c_client)
{
	...
	pr_err("stephen msm_camera_power_up init_done1 = %d\n",init_done);
/*
	if(init_done > 0){
		init_done++;
	pr_err("stephen msm_camera_power_up init_done2 = %d\n",init_done);
		return 0;
	}
*/
//add ++
	...
	pr_err("stephen the camera pwr cnt=%d\n",camera_pwr_cnt);
...
//add --
	...
	CDBG("%s exit\n", __func__);
	init_done = 1;
pr_err("msm_camera_power_up init_done =%d\n",init_done);
	return 0;
power_up_failed:
	pr_err("%s:%d failed\n", __func__, __LINE__);
...
//add ++
    char cmd_path[] = "/system/bin/setprop";        
    char* cmd_argv[] = {cmd_path,"persist.camera.release","1",NULL};
	...
	struct msm_camera_i2c_client *sensor_i2c_client)
{
	...
	if (!ctrl || !sensor_i2c_client) {
		...
		pr_err("stepen msm_camera_power_down init_done =%d\n",init_done);
/*
	if(init_done > 1){
		init_done--;
		pr_err("stepen msm_camera_power_down init_done =%d\n",init_done);
...
*/
	...
	if(camera_pwr_cnt == 0){
		//camera_pwr_set(0);
		set_camrs_flag(1);//the lost has release
		//ret = call_usermodehelper(cmd_path, cmd_argv, cmd_envp, UMH_WAIT_PROC);
pr_err("stephen sent broadcast start no 0\n");
		ret = call_usermodehelper(cmd_path, cmd_argv, cmd_envp, UMH_NO_WAIT);
pr_err("stephen sent broadcast end\n");
		if(ret < 0)
			pr_err("error to running user helper\n");
		
	}
	pr_err("stephen power_down camera_pwr_cnt=%d\n",camera_pwr_cnt);

	if(init_done == 1){
		pr_err("msm_camera_power_down ok ,never power down\n");
		return 0;
	}
	if (device_type == MSM_CAMERA_PLATFORM_DEVICE)
	...
	for (index = 0; index < ctrl->power_down_setting_size; index++) {
		...
		switch (pd->seq_type) {
		case SENSOR_CLK:
			pr_err("msm_camera_power_down to dia=sable clk\n");
		...
		case SENSOR_GPIO:
			...
			if (!ctrl->gpio_conf->gpio_num_info->valid
				[pd->seq_val])
				continue;
			pr_err("msm_camera_power_down to set gpio value");
			gpio_set_value_cansleep(
				ctrl->gpio_conf->gpio_num_info->gpio_num
				[pd->seq_val],
				(int) pd->config_val);
	...
	ctrl->cam_pinctrl_status = 0;
	pr_err("stephen msm_camera_power_down to release gpio");
	msm_camera_request_gpio_table(
		ctrl->gpio_conf->cam_gpio_req_tbl,
		ctrl->gpio_conf->cam_gpio_req_tbl_size, 0);
	CDBG("%s exit\n", __func__);
	...
	if(init_done == 1){
	init_done = 0;
	pr_err("stephen power down initdone to 0\n");
	}
	*/
	return 0;
}
