...
pr_err("stephen msm_isp_axi_create_stream stream_cfg_cmd->stream_src=0x%x\n",stream_cfg_cmd->stream_src);
	if (i >= VFE_AXI_SRC_MAX) {
		pr_err("%s:%d invalid stream_src %d\n", __func__, __LINE__,
			stream_cfg_cmd->stream_src);
		return -EINVAL;
	}
pr_err("stephen in msm_isp_axi_create_stream axi_data->stream_info[%d].state=0x%x\n",i,axi_data->stream_info[i].state);
	...
	ISP_DBG("%s: vfe %d handle %x\n", __func__, vfe_dev->pdev->id,
		stream_cfg_cmd->axi_stream_handle);
//	pr_err("%s: stephen vfe %d handle %x\n", __func__, vfe_dev->pdev->id,
//		stream_cfg_cmd->axi_stream_handle);
...
//pr_err("stephen msm_isp_validate_axi_request out format=%d\n,(int)stream_cfg_cmd->output_format");
	...
	case V4L2_PIX_FMT_Y12:
		...
		pr_err("1 num_planes V4L2_PIX_FMT_Y12\n");
	...
	case V4L2_PIX_FMT_NV41:
		pr_err("2 ==num_planes V4L2_PIX_FMT_NV41\n");
	...
	case V4L2_PIX_FMT_NV61:
		pr_err("2 num_planes V4L2_PIX_FMT_NV61\n");
		stream_info->num_planes = 2;
		//stream_info->num_planes = 1;
	...
	case V4L2_PIX_FMT_NV42:
		pr_err("2 --num_planes V4L2_PIX_FMT_NV42\n");
		stream_info->num_planes = 2;
		//stream_info->num_planes = 1;
...
int msm_isp_request_axi_stream(struct vfe_device *vfe_dev, void *arg)
{
	...
	if (stream_info->stream_src < RDI_INTF_0) {
		...
		pr_err("stephen io_format=0x%x\n",io_format);
		pr_err("stephen stream_info->output_format=0x%x\n",stream_info->output_format);
		if (stream_info->stream_src == CAMIF_RAW ||
			...
			if (stream_info->stream_src == CAMIF_RAW &&
				...
				pr_err("%s:stephen Overriding input format\n",
					__func__);
			io_format = stream_info->output_format;
		}
	...
	for (i = 0; i < VFE_AXI_SRC_MAX; i++) {
		if (SRC_TO_INTF(axi_data->stream_info[i].stream_src) !=
			...
			ISP_DBG("%s stream_src %d frame_src %d\n", __func__,
				...
				frame_src);
	/*		pr_err("%s stream_src %d frame_src %d\n", __func__,
				SRC_TO_INTF(
				axi_data->stream_info[i].stream_src),
				frame_src);
	*/	
...
//pr_err("stephen stream_update[frame_src] = %d,frame_src=%d\n",vfe_dev->axi_data.stream_update[frame_src],frame_src);
	if (vfe_dev->axi_data.stream_update[frame_src] == 0)
	{
		//pr_err("stream_config_complete\n");
...
//pr_err("stephen vfe_dev->axi_data.num_active_stream=%d \n",vfe_dev->axi_data.num_active_stream);
//pr_err("stephen stream_cfg_cmd->num_streams =%d \n",stream_cfg_cmd->num_streams);
//pr_err("stephen stream_cfg_cmd->cmd =0x%x \n",stream_cfg_cmd->cmd);
	if (vfe_dev->axi_data.num_active_stream == stream_cfg_cmd->num_streams
		&& (stream_cfg_cmd->cmd == STOP_STREAM ||
		stream_cfg_cmd->cmd == STOP_IMMEDIATELY))
		{
		//pr_err("stephen set halt 1\n");
		*halt = 1;
		}
	else{
		//pr_err("stephen set halt 0\n");
		*halt = 0;
	}

	if ((pix_stream_cnt) &&
		(axi_data->src_info[VFE_PIX_0].input_mux != EXTERNAL_READ)) {
		//pr_err("stephen input_mux != EXTERNAL_READ\n");
		if (cur_pix_stream_cnt == 0 && pix_stream_cnt &&
			stream_cfg_cmd->cmd == START_STREAM)
			{
			*camif_update = ENABLE_CAMIF;
			//pr_err("stephen set to camif_update = ENABLE_CAMIF\n");
			}
			...
			stream_cfg_cmd->cmd == STOP_IMMEDIATELY)) {
			if (*halt){
				//pr_err("stephen halt to DISABLE_CAMIF_IMME\n");
				*camif_update = DISABLE_CAMIF_IMMEDIATELY;
			}
			else {
				//pr_err("stephen halt to DISABLE_CAMIF\n");
				*camif_update = DISABLE_CAMIF;
			}
		} else {
				//pr_err("stephen NO_UPDATE 1\n");
			*camif_update = NO_UPDATE;
		}
	} else {
		//pr_err("stephen NO_UPDATE 2\n");
		*camif_update = NO_UPDATE;
	}
}
...
//	pr_err("stephen wait_for_cfg_done src_mask =0x%x\n",src_mask);
	if (src_mask) {
...
static int msm_isp_axi_wait_for_cfg_fastmode(struct vfe_device *vfe_dev,
	enum msm_isp_camif_update_state camif_update,
	uint32_t src_mask, int regUpdateCnt)
{
	int rc;
			...
			int halt)
{
	...
	for (i = 0; i < stream_cfg_cmd->num_streams; i++) {
		...
		if (stream_info->num_planes > 1){
			//pr_err("stephen stream_info->num_planes > 1\n");
			vfe_dev->hw_info->vfe_ops.axi_ops.
		...
		/*
		pr_err("%s: stream 0x%x, vfe %d camif %d halt %d wait %d\n",
			__func__,
			stream_info->stream_id,
			vfe_dev->pdev->id,
			camif_update,
		...
		*/
		...
		if (!wait_for_complete_for_this_stream ||
			...
			src_mask |= (1 << intf);
			//pr_err("stephen src_mask=0x%x\n",src_mask);
		}

	}
//pr_err("stephen src_mask=0x%x\n",src_mask);
	if (src_mask) {
		pr_err("stephen to stop stream\n");
		rc = msm_isp_axi_wait_for_cfg_fastmode(vfe_dev, camif_update,
			src_mask, 2);
		if (rc < 0) {
	...
	for (i = 0; i < stream_cfg_cmd->num_streams; i++) {
		...
		for (bufq_id = 0; bufq_id < VFE_BUF_QUEUE_MAX; bufq_id++) {
			...
			//pr_err("stephen to ops->flush_buf\n");
			rc = vfe_dev->buf_mgr->ops->flush_buf(
				vfe_dev->buf_mgr, vfe_dev->pdev->id,
				bufq_handle, MSM_ISP_BUFFER_FLUSH_ALL,
				&timestamp.buf_time, 0);
				//pr_err("stephen to ops->flush_buf rc=0x%x\n",rc);
			if (rc == -EFAULT) {
				msm_isp_halt_send_error(vfe_dev,
					ISP_EVENT_BUF_FATAL_ERROR);
				return rc;
...
int msm_isp_update_axi_stream(struct vfe_device *vfe_dev, void *arg)
{
	...
	case UPDATE_STREAM_REMOVE_BUFQ: {
		for (i = 0; i < update_cmd->num_streams; i++) {
			...
			if (stream_info->state == ACTIVE) {
				stream_info->state = UPDATING;
				pr_err("stephen UPDATE_STREAM_REMOVE_BUFQ\n");
				rc = msm_isp_axi_wait_for_cfg_done(vfe_dev,
...
//pr_err("stephen stream_info->num_planes = %d\n",stream_info->num_planes);
	spin_lock_irqsave(&stream_info->lock, flags);
	pingpong_bit = (~(pingpong_status >> stream_info->wm[0]) & 0x1);
	for (i = 0; i < stream_info->num_planes; i++) {
		if (pingpong_bit !=
