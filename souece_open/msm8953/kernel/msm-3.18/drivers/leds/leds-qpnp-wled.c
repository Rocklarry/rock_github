...
struct qpnp_wled {
	...
	u32 backlight_level_coefficient;
	u16 ctrl_base;
	u16 sink_base;
	u16 mod_freq_khz;
	u16 hyb_thres;
				...
				enum led_brightness level)
{
	...
	wled->cdev.brightness = level * wled->backlight_level_coefficient;
...
static int qpnp_wled_parse_dt(struct qpnp_wled *wled)
{
	...
	rc = of_property_read_u32(spmi->dev.of_node,
			"qcom,backlight_level_coefficient", &temp_val);
	if (!rc) {
		wled->backlight_level_coefficient = temp_val;
		dev_dbg(&spmi->dev, "Use pmi8952: set backlight level coefficient to [%d]\n",wled->backlight_level_coefficient);
	} else {
		wled->backlight_level_coefficient = 1;
		dev_err(&spmi->dev, "Not use pmi8952, set backlight level coefficient to default [%d]\n",wled->backlight_level_coefficient);
	}
