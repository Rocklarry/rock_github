 ...
 *      V1.6:
 *          1. replace guitar_client with client;
...
static s32 gup_init_panel(struct goodix_ts_data *ts)
{
    ...
    u8 *config_data[GOODIX_CFG_GROUP_NUM];
    s32 ret = 0;
    s32 i = 0, n = 0;
    u8 check_sum = 0;
    u8 opr_buf[16];
    ...
    for (i = 0; i < GOODIX_MAX_CFG_GROUP; i++) {
        for (n = 0; i < GOODIX_CFG_GROUP_NUM; n++) {
            if (ts->pdata->config_data_len[i][n])
                break;
        }
    }
    ...
    for (n = 0; n < GOODIX_CFG_GROUP_NUM; n++) {
        if (ts->pdata->config_data_len[sensor_id][n] < GTP_CONFIG_MIN_LENGTH ||
            ...
            dev_info(&client->dev, "Valid cfg-data%d-%d , config length: %d", (int)sensor_id, n, (int)ts->pdata->config_data_len[sensor_id][n]);//modify stephen
        }
    }
                    ...
                    const struct firmware *cfg)
{
    ...
    pr_debug("config file ASCII len:%d", (int)cfg->size);
    pr_debug("need config binary len:%d", (int)chip_cfg_len);
        ...
        struct st_update_msg *msg, u8 *path)
{
    ...
    dev_dbg(&client->dev, "Config File: %s size=%d", path, (int)fw->size);//modify stephen
    msg->fw_data =
...
s32 gup_update_proc(void *dir, struct i2c_client *client)
{
    s32 ret = 0;
    u8 retry = 0;
    struct st_fw_head fw_head;
    ...
    ret = gup_check_update_file(client, &fw_head, (u8 *)dir);
    if (ret == FAIL) {
...
#endif
    ret = gup_enter_update_mode(client);
    if (ret == FAIL) {
...
static void gup_update_work(struct work_struct *work)
{
    ...
    struct goodix_ts_data *ts =
            container_of(delay_work, struct goodix_ts_data, goodix_update_work);

    if (gup_update_proc(NULL, ts->client) == FAIL)
