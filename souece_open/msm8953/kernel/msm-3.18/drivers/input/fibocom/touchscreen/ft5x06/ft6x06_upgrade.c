static u8 ft6x06_ctpm_get_i_file_ver(struct ft5x06_ts_data *ts)
{
    u16 ui_sz;
...
static int ft6x06_get_upgrade_info(struct i2c_client *client, struct fw_upgrade_info *upgrade_info)
{
    ...
    case FT6X06_ID:
        ...
        upgrade_info->delay_erase_flash = FT6306_UPGRADE_EARSE_DELAY;
        printk("[FST] Device type IC_FT6306.\n");
    break;
    default:
...
[return]:
    ...
    ERR_ECC        :ecc error.
*/
static E_UPGRADE_ERR_TYPE  ft6x06_ctpm_fw_upgrade(struct i2c_client *client, u8* pbt_buf, u32 dw_lenth)
{
    u8 reg_val[2] = {0};
    u32 i = 0;
    u32 packet_number;
    ...
    if(ft6x06_get_upgrade_info(client, &upgradeinfo)){
...
static int ft6x06_ctpm_fw_upgrade_with_i_file(struct i2c_client *client)
{
    u8* pbt_buf = 0;
    int i_ret, buf_size;
    ...
    i_ret =  ft6x06_ctpm_fw_upgrade(client, pbt_buf, buf_size);
    if (i_ret != 0)
    {
