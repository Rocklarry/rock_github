	...
	if (ts->pdata->driver_send_cfg) {
		for (i = 0; i < GOODIX_MAX_CFG_GROUP; i++)
			dev_dbg(&client->dev, "Config Groups(%d) Lengths: %d",
				i, (int)ts->pdata->config_data_len[i]);//modify stephen
...
printk("sephen sc820 gtp_check_product_id ==product_id = %s,ts->pdata->product_id= %s===\n",product_id,ts->pdata->product_id);
			...
			struct goodix_ts_platform_data *pdata)
{
	...
	for (i = 0; i < GOODIX_MAX_CFG_GROUP; i++) {
		...
		prop = of_find_property(np, prop_name,
			(int *)&pdata->config_data_len[i]);//modify stephen
		if (!prop || !prop->value) {
