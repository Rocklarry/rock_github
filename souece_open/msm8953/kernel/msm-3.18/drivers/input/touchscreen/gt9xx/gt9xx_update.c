					...
					const struct firmware *cfg)
{
	...
	pr_debug("config file ASCII len:%d", (int)cfg->size);//stephen
	pr_debug("need config binary len:%d", (int )chip_cfg_len);//stephene
		...
		struct st_update_msg *msg, u8 *path)
{
	...
	dev_dbg(&client->dev, "Config File: %s size=%d", path, (int)fw->size);
	msg->fw_data =
