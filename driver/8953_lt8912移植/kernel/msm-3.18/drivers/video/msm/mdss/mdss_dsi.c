...
//A: wming add gpio136 ctl @{
static struct mdss_dsi_ctrl_pdata *s_ctrl_pdata;
static bool gpio136_required = false;
//@}
...
static int mdss_dsi_panel_power_on(struct mdss_panel_data *pdata)
{
	...
	if (ret) {
		pr_err("==rrd== %s: failed to enable vregs for %s\n",
			__func__, __mdss_dsi_pm_name(DSI_PANEL_PM));
		return ret;
	}
	 ...
	 */
	 pr_err("==rrd==  stephen go to check if need panel reset\n");
	if (pdata->panel_info.cont_splash_enabled ||
		!pdata->panel_info.mipi.lp11_init) {
		if (mdss_dsi_pinctrl_set_state(ctrl_pdata, true))
			pr_err("==rrd== reset enable: pinctrl not enabled\n");
	...
	int power_state)
{
	...
	//struct mdss_dsi_ctrl_pdata *ctrl_hdmi;
	...
	case MDSS_PANEL_POWER_OFF:
		pr_err("==rrd== to powen down hdmi start=====\n");
		ret = mdss_dsi_panel_power_off(pdata);
		break;
	case MDSS_PANEL_POWER_ON:
		if (mdss_dsi_is_panel_on_lp(pdata))
		{
			pr_err("==rrd==  ===pown on but power lp\n");
			ret = mdss_dsi_panel_power_lp(pdata, false);
		}else{
			pr_err("==rrd== ===pown on but power on===\n");
			ret = mdss_dsi_panel_power_on(pdata);
		}
		//pr_err("==rrd== to powen on hdmi start=no need====\n");
		//mdss_mipi_i2c_init(ctrl_hdmi,0);
		pr_err("==rrd== to powen on hdmi end==no need===\n");
		break;
...
int mdss_dsi_on(struct mdss_panel_data *pdata)
{
	...
	mipi = &pdata->panel_info.mipi;
	 ...
	 pr_err("==rrd== to checkout if lcd is on\n");
	if (mdss_dsi_is_panel_on_interactive(pdata)) {
		...
		pr_err("==rrd==  %s: panel already on\n", __func__);
		goto end;
	}
	pr_err("==rrd== start to power onn\n");
	...
	pr_err("==rrd== start to check if force_clk_lane_hs\n");

	//if (mipi->force_clk_lane_hs) 
	if(1){
		u32 tmp;
		pr_err("==rrd==  =====get force_clk_lane_hs is ok");
		tmp = MIPI_INP((ctrl_pdata->ctrl_base) + 0xac);
		tmp |= (1<<28);
		MIPI_OUTP((ctrl_pdata->ctrl_base) + 0xac, tmp);
		wmb();
	}else{
		pr_err("==rrd== force_clk_lane_hs is false\n");
	}
...
static void __mdss_dsi_calc_dfps_delay(struct mdss_panel_data *pdata)
{
	...
	pr_err("==rrd== start to checkout pinfo->mipi.force_clk_lane_hs=%d\n",pinfo->mipi.force_clk_lane_hs);
				  ...
				  int event, void *arg)
{
	...
	case MDSS_EVENT_DSI_CMDLIST_KOFF:
		pr_err("==rrd== to MDSS_EVENT_DSI_CMDLIST_KOFF\n");
		...
		struct platform_device *pdev)
{
	...
	pr_err("==rrd== %s:%d: Select primary panel from dt\n",
					__func__, __LINE__);
	dsi_pan_node = of_parse_phandle(pdev->dev.of_node,
					"qcom,dsi-pref-prim-pan", 0);
	if (!dsi_pan_node)
		pr_err("==rrd== %s:can't find panel phandle\n", __func__);
		...
		struct platform_device *pdev, char *panel_cfg)
{
	...
	struct mdss_panel_info *pinfo = &ctrl_pdata->panel_data.panel_info;
pr_err("==rrd== go to mdss_dsi_find_panel_of_node panel_cfg =%s\n",panel_cfg);
	...
	if (!len) {
		...
		if (override_cfg) {
			pr_err("==rrd== really has the override cfg\n");
			*override_cfg = '\0';
		...
		if ((str1 != panel_cfg) && (*(str1-1) != ':')) {
			...
			pr_err("==rrd== false match with config node name in \"%s\". search again in \"%s\"\n",
				panel_cfg, str1);
		...
		pr_err("==rrd== get the pamel name str2=%s",str2);
		if (!str2) {
			pr_err("==rrd== must the panel name is null\n");
	...
	int ndx)
{
	...
	pr_err("==rrd== in mdss_dsi_config_panel\n");
	if (!ctrl_pdata) {
...
//A: wming add gpio136 ctl @{
static ssize_t gpio136_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
	unsigned char rec_data;
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = s_ctrl_pdata;
...
static struct file_operations gpio136_fops = {
	.owner = THIS_MODULE,
	.write = gpio136_write,
};
	
static struct miscdevice gpio136_misc = {
...
static int mdss_dsi_ctrl_probe(struct platform_device *pdev)
{
	...
	pr_err("==rrd== go to mdss_dsi_ctrl_probe\n");
	if (index == 0)
	{
		pdev->id = 1;
		pr_err("==rrd== go to mdss_dsi_ctrl_probei index 1\n");
	}
	else
	{
		pr_err("==rrd== go to mdss_dsi_ctrl_probei index 2\n");
		pdev->id = 2;
	}
	...
	//A: wming add gpio136 ctl @{
	if (!gpio136_required) {
	    rc = misc_register(&gpio136_misc);
	    if (rc) {
		    pr_err("%s GPIO136 miscdevice register failed", __func__);
	    }
		
		gpio136_required = true;
	}
	//@}
