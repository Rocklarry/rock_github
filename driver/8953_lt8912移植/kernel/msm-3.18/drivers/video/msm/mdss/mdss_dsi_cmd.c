				...
				struct dcs_cmd_req *cmdreq)
{
	...
	*req = *cmdreq;
	...
	pr_err("==rrd== %s: tot=%d put=%d get=%d\n", __func__,
		clist->tot, clist->put, clist->get);
	pr_err("==rrd== cmdlist_commit not implemented!\n");
	mutex_unlock(&ctrl->cmdlist_mutex);

	if (req->flags & CMD_REQ_COMMIT) {
		if (!ctrl->cmdlist_commit)
			pr_err("==rrd== sss cmdlist_commit not implemented!\n");
		else
		{
			pr_err("cmdlist_commit ==rrd== start implemented!\n");
