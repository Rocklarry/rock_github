...
public class HttpService{
    private final String TAG = "===rock=== HTTP";
    ...
    public void sendHttpRequest(String url, String json) {
        ...
        if (g.getRestServer() == null) {
            ...
            Log.i(TAG, "Rest server IP is " + g.getRestServer());//获取设备IP地址 192.168.125.1
            String full_url = "http://" + g.getRestServer() + ":8000" + url;
            Log.i(TAG, "Request URL " + full_url); //访问的地址 http://192.168.125.1:8000/settings/network/wireless_access_points
            if (json != null) {
                Log.i(TAG, "Dan receive a post request. Post data is " + json);//连接的wifi 名称 密码 Dan receive a post request. Post data is {"ssid" : "PDCN","password" : "abcd123456"}
                ...
                public void onResponse(Call call, Response response) throws IOException {
                    if (response.isSuccessful()) {
                        ...
                        activity.runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                Log.d(TAG, "Get reponse string " + responseStr);//设备搜索到的ssid列表

                                if (request.url().toString().compareTo(aplistURL) == 0) {
                                    Log.d(TAG, "Get APList " + responseStr);
                                    responseAPList(responseStr);//显示ssid列表
                                }
                                else {
